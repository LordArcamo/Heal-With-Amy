...
// node_modules/@vitest/runner/node_modules/tinyrainbow/dist/index.js
var b = {
  reset: [0, 0],
...
function _mergeNamespaces(n, m) {
  m.forEach(function(e) {
    e && typeof e !== "string" && !Array.isArray(e) && Object.keys(e).forEach(function(k) {
      if (k !== "default" && !(k in n)) {
        var d4 = Object.getOwnPropertyDescriptor(e, k);
        Object.defineProperty(n, k, d4.get ? d4 : {
          enumerable: true,
          get: function() {
            return e[k];
          }
...
function printIteratorEntries(iterator, config2, indentation, depth, refs, printer4, separator = ": ") {
  let result = "";
  let width = 0;
  ...
  if (!current.done) {
    ...
    while (!current.done) {
      ...
      const name = printer4(current.value[0], config2, indentationNext, depth, refs);
      const value = printer4(current.value[1], config2, indentationNext, depth, refs);
...
function printIteratorValues(iterator, config2, indentation, depth, refs, printer4) {
  let result = "";
  let width = 0;
  let current = iterator.next();
  if (!current.done) {
    ...
    while (!current.done) {
      ...
      result += printer4(current.value, config2, indentationNext, depth, refs);
      current = iterator.next();
      if (!current.done) {
        result += `,${config2.spacingInner}`;
      } else if (!config2.min) {
...
function printListItems(list, config2, indentation, depth, refs, printer4) {
  let result = "";
  ...
  if (length > 0) {
    ...
    for (let i4 = 0; i4 < length; i4++) {
      result += indentationNext;
      if (i4 === config2.maxWidth) {
        result += "…";
        break;
      }
      if (isDataView(list) || i4 in list) {
        result += printer4(isDataView(list) ? list.getInt8(i4) : list[i4], config2, indentationNext, depth, refs);
      }
      if (i4 < length - 1) {
        result += `,${config2.spacingInner}`;
      } else if (!config2.min) {
...
function printObjectProperties(val, config2, indentation, depth, refs, printer4) {
  let result = "";
  ...
  if (keys2.length > 0) {
    ...
    for (let i4 = 0; i4 < keys2.length; i4++) {
      const key = keys2[i4];
      const name = printer4(key, config2, indentationNext, depth, refs);
      const value = printer4(val[key], config2, indentationNext, depth, refs);
      result += `${indentationNext + name}: ${value}`;
      if (i4 < keys2.length - 1) {
        result += `,${config2.spacingInner}`;
      } else if (!config2.min) {
        result += ",";
      }
...
var SPACE$2 = " ";
var serialize$5 = (val, config2, indentation, depth, refs, printer4) => {
  const stringedValue = val.toString();
  if (stringedValue === "ArrayContaining" || stringedValue === "ArrayNotContaining") {
    if (++depth > config2.maxDepth) {
      return `[${stringedValue}]`;
    }
    return `${stringedValue + SPACE$2}[${printListItems(val.sample, config2, indentation, depth, refs, printer4)}]`;
  }
  if (stringedValue === "ObjectContaining" || stringedValue === "ObjectNotContaining") {
    if (++depth > config2.maxDepth) {
      return `[${stringedValue}]`;
    }
    return `${stringedValue + SPACE$2}{${printObjectProperties(val.sample, config2, indentation, depth, refs, printer4)}}`;
  }
  if (stringedValue === "StringMatching" || stringedValue === "StringNotMatching") {
    return stringedValue + SPACE$2 + printer4(val.sample, config2, indentation, depth, refs);
  }
...
var serialize$4 = (collection, config2, indentation, depth, refs, printer4) => {
  ...
  return (config2.min ? "" : name + SPACE$1) + (OBJECT_NAMES.has(name) ? `{${printObjectProperties(isNamedNodeMap(collection) ? [...collection].reduce((props, attribute) => {
    ...
    return props;
  }, {}) : { ...collection }, config2, indentation, depth, refs, printer4)}}` : `[${printListItems([...collection], config2, indentation, depth, refs, printer4)}]`);
};
var plugin$4 = {
...
function printProps(keys2, props, config2, indentation, depth, refs, printer4) {
  const indentationNext = indentation + config2.indent;
  ...
  return keys2.map((key) => {
    ...
    let printed = printer4(value, config2, indentationNext, depth, refs);
...
function printChildren(children, config2, indentation, depth, refs, printer4) {
  return children.map((child) => config2.spacingOuter + indentation + (typeof child === "string" ? printText(child, config2) : printer4(child, config2, indentation, depth, refs))).join("");
}
function printShadowRoot(children, config2, indentation, depth, refs, printer4) {
  if (config2.printShadowRoot === false) {
    return "";
  }
  return [`${config2.spacingOuter + indentation}#shadow-root`, printChildren(children, config2, indentation + config2.indent, depth, refs, printer4)].join("");
}
...
function serializeDOM(node, config2, indentation, depth, refs, printer4, filterNode) {
  if (nodeIsText(node)) {
  ...
  return printElement(type3, printProps(nodeIsFragment(node) ? [] : Array.from(node.attributes, (attr) => attr.name).sort(), nodeIsFragment(node) ? {} : [...node.attributes].reduce((props, attribute) => {
    props[attribute.name] = attribute.value;
    return props;
  }, {}), config2, indentation + config2.indent, depth, refs, printer4), (resolvedShadowChildren.length > 0 ? printShadowRoot(resolvedShadowChildren, config2, indentation + config2.indent, depth, refs, printer4) : "") + printChildren(resolvedChildren, config2, indentation + config2.indent, depth, refs, printer4), config2, indentation);
}
var serialize$3 = (node, config2, indentation, depth, refs, printer4) => serializeDOM(node, config2, indentation, depth, refs, printer4);
function createDOMElementFilter(filterNode) {
  return {
    test: test$3,
    serialize: (node, config2, indentation, depth, refs, printer4) => serializeDOM(node, config2, indentation, depth, refs, printer4, filterNode)
  };
}
var plugin$3 = {
...
var SPACE = " ";
var LAZY = "…";
function printImmutableEntries(val, config2, indentation, depth, refs, printer4, type3) {
  return ++depth > config2.maxDepth ? printAsLeaf(getImmutableName(type3)) : `${getImmutableName(type3) + SPACE}{${printIteratorEntries(val.entries(), config2, indentation, depth, refs, printer4)}}`;
}
function getRecordEntries(val) {
  let i4 = 0;
  return { next() {
    if (i4 < val._keys.length) {
      const key = val._keys[i4++];
      return {
        done: false,
...
function printImmutableRecord(val, config2, indentation, depth, refs, printer4) {
  const name = getImmutableName(val._name || "Record");
  return ++depth > config2.maxDepth ? printAsLeaf(name) : `${name + SPACE}{${printIteratorEntries(getRecordEntries(val), config2, indentation, depth, refs, printer4)}}`;
}
function printImmutableSeq(val, config2, indentation, depth, refs, printer4) {
  const name = getImmutableName("Seq");
  if (++depth > config2.maxDepth) {
    return printAsLeaf(name);
  }
  if (val[IS_KEYED_SENTINEL]) {
    return `${name + SPACE}{${val._iter || val._object ? printIteratorEntries(val.entries(), config2, indentation, depth, refs, printer4) : LAZY}}`;
  }
  return `${name + SPACE}[${val._iter || val._array || val._collection || val._iterable ? printIteratorValues(val.values(), config2, indentation, depth, refs, printer4) : LAZY}]`;
}
function printImmutableValues(val, config2, indentation, depth, refs, printer4, type3) {
  return ++depth > config2.maxDepth ? printAsLeaf(getImmutableName(type3)) : `${getImmutableName(type3) + SPACE}[${printIteratorValues(val.values(), config2, indentation, depth, refs, printer4)}]`;
}
var serialize$2 = (val, config2, indentation, depth, refs, printer4) => {
  if (val[IS_MAP_SENTINEL]) {
    return printImmutableEntries(val, config2, indentation, depth, refs, printer4, val[IS_ORDERED_SENTINEL] ? "OrderedMap" : "Map");
  }
  if (val[IS_LIST_SENTINEL]) {
    return printImmutableValues(val, config2, indentation, depth, refs, printer4, "List");
  }
  if (val[IS_SET_SENTINEL]) {
    return printImmutableValues(val, config2, indentation, depth, refs, printer4, val[IS_ORDERED_SENTINEL] ? "OrderedSet" : "Set");
  }
  if (val[IS_STACK_SENTINEL]) {
    return printImmutableValues(val, config2, indentation, depth, refs, printer4, "Stack");
  }
  if (val[IS_SEQ_SENTINEL]) {
    return printImmutableSeq(val, config2, indentation, depth, refs, printer4);
  }
  return printImmutableRecord(val, config2, indentation, depth, refs, printer4);
};
...
function requireReactIs_production_min() {
  ...
  var b4 = Symbol.for("react.element"), c = Symbol.for("react.portal"), d4 = Symbol.for("react.fragment"), e = Symbol.for("react.strict_mode"), f = Symbol.for("react.profiler"), g = Symbol.for("react.provider"), h = Symbol.for("react.context"), k = Symbol.for("react.server_context"), l = Symbol.for("react.forward_ref"), m = Symbol.for("react.suspense"), n = Symbol.for("react.suspense_list"), p = Symbol.for("react.memo"), q = Symbol.for("react.lazy"), t = Symbol.for("react.offscreen"), u;
  ...
  reactIs_production_min.Element = b4;
  reactIs_production_min.ForwardRef = l;
  reactIs_production_min.Fragment = d4;
  ...
  reactIs_production_min.isElement = function(a) {
    return "object" === typeof a && null !== a && a.$$typeof === b4;
  };
  ...
  reactIs_production_min.isValidElementType = function(a) {
    return "string" === typeof a || "function" === typeof a || a === d4 || a === f || a === e || a === m || a === n || a === t || "object" === typeof a && null !== a && (a.$$typeof === q || a.$$typeof === p || a.$$typeof === g || a.$$typeof === h || a.$$typeof === l || a.$$typeof === u || void 0 !== a.getModuleId) ? true : false;
  };
...
var serialize$1 = (element, config2, indentation, depth, refs, printer4) => ++depth > config2.maxDepth ? printElementAsLeaf(getType(element), config2) : printElement(getType(element), printProps(getPropKeys$1(element), element.props, config2, indentation + config2.indent, depth, refs, printer4), printChildren(getChildren(element.props.children), config2, indentation + config2.indent, depth, refs, printer4), config2, indentation);
...
var serialize = (object2, config2, indentation, depth, refs, printer4) => ++depth > config2.maxDepth ? printElementAsLeaf(object2.type, config2) : printElement(object2.type, object2.props ? printProps(getPropKeys(object2), object2.props, config2, indentation + config2.indent, depth, refs, printer4) : "", object2.children ? printChildren(object2.children, config2, indentation + config2.indent, depth, refs, printer4) : "", config2, indentation);
...
var ErrorPlugin = {
  ...
  serialize(val, config2, indentation, depth, refs, printer4) {
    if (refs.includes(val)) {
      return "[Circular]";
    }
    refs = [...refs, val];
    ...
    return hitMaxDepth ? `[${name}]` : `${name} {${printIteratorEntries(Object.entries(entries).values(), config2, indentation, depth, refs, printer4)}}`;
  }
};
...
function printer(val, config2, indentation, depth, refs, hasCalledToJSON) {
  ...
  if (plugin5 !== null) {
    result = printPlugin(plugin5, val, config2, indentation, depth, refs);
  } else {
  ...
  (_a = config2._outputLengthPerDepth)[depth] ?? (_a[depth] = 0);
...
// node_modules/@vitest/runner/node_modules/@vitest/utils/dist/display.js
var ansiColors = {
  bold: ["1", "22"],
  dim: ["2", "22"],
  italic: ["3", "23"],
...
function inspectList(list, options, inspectItem, separator = ", ") {
  ...
  for (let i4 = 0; i4 < size; i4 += 1) {
    const last = i4 + 1 === list.length;
    const secondToLast = i4 + 2 === list.length;
    ...
    peek = last ? "" : inspectItem(list[i4 + 1], options) + (secondToLast ? "" : separator);
...
function inspectTypedArray(array2, options) {
  ...
  for (let i4 = 0; i4 < array2.length; i4++) {
    const string2 = `${options.stylize(truncate(array2[i4], options.truncate), "number")}${i4 === array2.length - 1 ? "" : ", "}`;
    options.truncate -= string2.length;
    if (array2[i4] !== array2.length && options.truncate <= 3) {
      output += `${truncator}(${array2.length - array2[i4] + 1})`;
      break;
    }
    output += string2;
  }
...
var isNaN = Number.isNaN || ((i4) => i4 !== i4);
...
function createNodeFilterFromSelector(selector) {
  const ELEMENT_NODE4 = 1;
  const COMMENT_NODE4 = 8;
  return (node) => {
    if (node.nodeType === COMMENT_NODE4) {
      return false;
    }
    if (node.nodeType === ELEMENT_NODE4 && node.matches) {
      try {
...
function baseFormat(args, options = {}) {
  ...
  if (typeof args[0] !== "string") {
    ...
    for (let i5 = 0; i5 < args.length; i5++) {
      objects.push(formatArg(args[i5], {
        depth: 0,
        colors: false
      }));
    }
  ...
  let str = String(template).replace(formatRegExp, (x) => {
    ...
    switch (x) {
      ...
      case "%f":
        return Number.parseFloat(String(args[i4++])).toString();
      case "%o":
        return formatArg(args[i4++], {
          showHidden: true,
          showProxy: true
        });
      case "%O":
  ...
  for (let x = args[i4]; i4 < len; x = args[++i4]) {
...
// node_modules/@vitest/runner/node_modules/@vitest/utils/dist/constants.js
...
function requireBuild() {
  ...
  const extendPathsF = (d4, aEnd, bEnd, bF, isCommon, aIndexesF, iMaxF) => {
    let iF = 0;
    let kF = -d4;
    ...
    const nF = d4 < iMaxF ? d4 : iMaxF;
    for (iF += 1, kF += 2; iF <= nF; iF += 1, kF += 2) {
      if (iF !== d4 && aIndexPrev1 < aIndexesF[iF]) {
        aFirst = aIndexesF[iF];
      } else {
  ...
  const extendPathsR = (d4, aStart, bStart, bR, isCommon, aIndexesR, iMaxR) => {
    let iR = 0;
    let kR = d4;
    ...
    const nR = d4 < iMaxR ? d4 : iMaxR;
    for (iR += 1, kR -= 2; iR <= nR; iR += 1, kR -= 2) {
      if (iR !== d4 && aIndexesR[iR] < aIndexPrev1) {
        aFirst = aIndexesR[iR];
      } else {
        aFirst = aIndexPrev1 - 1;
        if (aFirst < aStart) {
  ...
  const extendOverlappablePathsF = (d4, aStart, aEnd, bStart, bEnd, isCommon, aIndexesF, iMaxF, aIndexesR, iMaxR, division) => {
    const bF = bStart - aStart;
    const aLength = aEnd - aStart;
    const bLength = bEnd - bStart;
    const baDeltaLength = bLength - aLength;
    const kMinOverlapF = -baDeltaLength - (d4 - 1);
    const kMaxOverlapF = -baDeltaLength + (d4 - 1);
    let aIndexPrev1 = NOT_YET_SET;
    const nF = d4 < iMaxF ? d4 : iMaxF;
    for (let iF = 0, kF = -d4; iF <= nF; iF += 1, kF += 2) {
      const insert = iF === 0 || iF !== d4 && aIndexPrev1 < aIndexesF[iF];
      ...
      if (kMinOverlapF <= kF && kF <= kMaxOverlapF) {
        const iR = (d4 - 1 - (kF + baDeltaLength)) / 2;
        if (iR <= iMaxR && aIndexesR[iR] - 1 <= aLast) {
          ...
          division.nChangePreceding = d4 - 1;
          if (d4 - 1 === aEndPreceding + bEndPreceding - aStart - bStart) {
          ...
          division.nChangeFollowing = d4 - 1;
          if (d4 - 1 === aEnd + bEnd - aStartFollowing - bStartFollowing) {
  ...
  const extendOverlappablePathsR = (d4, aStart, aEnd, bStart, bEnd, isCommon, aIndexesF, iMaxF, aIndexesR, iMaxR, division) => {
    const bR = bEnd - aEnd;
    const aLength = aEnd - aStart;
    const bLength = bEnd - bStart;
    const baDeltaLength = bLength - aLength;
    const kMinOverlapR = baDeltaLength - d4;
    const kMaxOverlapR = baDeltaLength + d4;
    let aIndexPrev1 = NOT_YET_SET;
    const nR = d4 < iMaxR ? d4 : iMaxR;
    for (let iR = 0, kR = d4; iR <= nR; iR += 1, kR -= 2) {
      const insert = iR === 0 || iR !== d4 && aIndexesR[iR] < aIndexPrev1;
      ...
      if (kMinOverlapR <= kR && kR <= kMaxOverlapR) {
        const iF = (d4 + (kR - baDeltaLength)) / 2;
        if (iF <= iMaxF && aLast - 1 <= aIndexesF[iF]) {
          const bLast = bFirst - nCommonR;
          division.nChangePreceding = d4;
          if (d4 === aLast + bLast - aStart - bStart) {
            division.aEndPreceding = aStart;
            division.bEndPreceding = bStart;
          } else {
  ...
  const divide = (nChange, aStart, aEnd, bStart, bEnd, isCommon, aIndexesF, aIndexesR, division) => {
    ...
    if (baDeltaLength % 2 === 0) {
      ...
      for (let d4 = 1; d4 <= dMax; d4 += 1) {
        iMaxF = extendPathsF(d4, aEnd, bEnd, bF, isCommon, aIndexesF, iMaxF);
        if (d4 < dMin) {
          iMaxR = extendPathsR(d4, aStart, bStart, bR, isCommon, aIndexesR, iMaxR);
        } else if (
...
// node_modules/@vitest/runner/node_modules/@vitest/utils/dist/serialize.js
var OBJECT_PROTO = Object.getPrototypeOf({});

// node_modules/@vitest/runner/node_modules/@vitest/utils/dist/timers.js
...
// node_modules/@vitest/runner/node_modules/@vitest/utils/dist/chunk-pathe.M-eThtNZ.js
...
var resolve = function(...arguments_) {
  ...
  for (let index5 = arguments_.length - 1; index5 >= -1 && !resolvedAbsolute; index5--) {
    const path = index5 >= 0 ? arguments_[index5] : cwd();
...
function normalizeString(path, allowAboveRoot) {
  ...
  let dots = 0;
  let char = null;
  for (let index5 = 0; index5 <= path.length; ++index5) {
    ...
    if (char === "/") {
      if (lastSlash === index5 - 1 || dots === 1) ;
        ...
        if (res.length > 0) {
          res += `/${path.slice(lastSlash + 1, index5)}`;
        } else {
          res = path.slice(lastSlash + 1, index5);
        }
...
function parseSingleFFOrSafariStack(raw) {
  ...
  for (let i4 = 0; i4 < line.length; i4++) {
    if (line[i4] === "@") {
      const candidateLocation = line.slice(i4 + 1);
      if (candidateLocation.includes(":") && candidateLocation.length >= 3) {
        atIndex = i4;
        locationPart = candidateLocation;
        functionName2 = i4 > 0 ? line.slice(0, i4) : void 0;
        break;
      }
    }
  }
...
var _lazyMatch = () => {
  var __lib__ = (() => {
    ...
    q(w, { default: () => re });
    var A = (r) => Array.isArray(r), d4 = (r) => typeof r == "function", Q = (r) => r.length === 0, W = (r) => typeof r == "number", K = (r) => typeof r == "object" && r !== null, X = (r) => r instanceof RegExp, b4 = (r) => typeof r == "string", h = (r) => r === void 0, Y = (r) => {
    ...
    }, i4 = (r, e) => A(r) ? er(r, e) : b4(r) ? ar(r, e) : nr(r, e), er = (r, e) => {
      const n = {};
      for (const a of r) {
      ...
      return (a) => {
        ...
        if (u > t) {
          if (!h(e) && !a.options.silent) {
            const s = a.input.slice(t, u), c = d4(e) ? e(s, o, String(t)) : e;
            h(c) || a.output.push(c);
          }
      ...
      return g((o) => {
        ...
        if (u) {
          if (!h(e) && !o.options.silent) {
            const s = d4(e) ? e(...u, o.input, String(o.index)) : e;
            h(s) || o.output.push(s);
          }
      ...
      if (n.input.startsWith(r, n.index)) {
        if (!h(e) && !n.options.silent) {
          const t = d4(e) ? e(r, n.input, String(n.index)) : e;
          h(t) || n.output.push(t);
        }
    ...
    }, tr = (r, e) => C4(r, 0, 1), f = (r, e) => C4(r, 0, 1 / 0), x = (r, e) => {
      const n = r.map(v);
      return g(_(M((a) => {
      ...
      if (d4(r)) return Q(r) ? E(r) : r;
      if (b4(r) || X(r)) return i4(r);
      if (A(r)) return x(r);
    ...
    }, sr = i4(/^\*\*\/\*$/, ".*"), cr = i4(/^\*\*\/(\*)?([ a-zA-Z0-9._-]+)$/, (r, e, n) => `.*${e ? "" : "(?:^|/)"}${n.replaceAll(".", "\\.")}`), lr = i4(/^\*\*\/(\*)?([ a-zA-Z0-9._-]*)\{([ a-zA-Z0-9._-]+(?:,[ a-zA-Z0-9._-]+)*)\}$/, (r, e, n, a) => `.*${e ? "" : "(?:^|/)"}${n.replaceAll(".", "\\.")}(?:${a.replaceAll(",", "|").replaceAll(".", "\\.")})`), y4 = i4(/\\./, p), pr = i4(/[$.*+?^(){}[\]\|]/, (r) => `\\${r}`), vr = i4(/./, p), hr = i4(/^(?:!!)*!(.*)$/, (r, e) => `(?!^${L(e)}$).*?`), dr = i4(/^(!!)+/, ""), fr = l([hr, dr]), xr = i4(/\/(\*\*\/)+/, "(?:/.+/|/)"), gr = i4(/^(\*\*\/)+/, "(?:^|.*/)"), mr = i4(/\/(\*\*)$/, "(?:/.*|$)"), _r = i4(/\*\*/, ".*"), j = l([xr, gr, mr, _r]), Sr = i4(/\*\/(?!\*\*\/)/, "[^/]*/"), yr = i4(/\*/, "[^/]*"), N = l([Sr, yr]), k = i4("?", "[^/]"), $r = i4("[", p), wr = i4("]", p), Ar = i4(/[!^]/, "^/"), br = i4(/[a-z]-[a-z]|[0-9]-[0-9]/i, p), Cr = i4(/[$.*+?^(){}[\|]/, (r) => `\\${r}`), Mr = i4(/[^\]]/, p), Er = l([y4, Cr, br, Mr]), B4 = x([$r, tr(Ar), f(Er), wr]), Pr = i4("{", "(?:"), Or = i4("}", ")"), Rr = i4(/(\d+)\.\.(\d+)/, (r, e, n) => or(+e, +n, Math.min(e.length, n.length)).join("|")), zr = i4(/([a-z]+)\.\.([a-z]+)/, (r, e, n) => R(e, n).join("|")), jr = i4(/([A-Z]+)\.\.([A-Z]+)/, (r, e, n) => R(e.toLowerCase(), n.toLowerCase()).join("|").toUpperCase()), Nr = l([Rr, zr, jr]), I = x([Pr, Nr, Or]), kr = i4("{", "(?:"), Br = i4("}", ")"), Ir = i4(",", "|"), Fr = i4(/[$.*+?^(){[\]\|]/, (r) => `\\${r}`), Lr = i4(/[^}]/, p), Zr = E(() => F), Dr = l([j, N, k, B4, I, Zr, y4, Fr, Ir, Lr]), F = x([kr, f(Dr), Br]), Ur = f(l([sr, cr, lr, fr, j, N, k, B4, I, F, y4, pr, vr])), Vr = Ur, Gr = z(Vr), L = Gr, Tr = i4(/\\./, p), qr = i4(/./, p), Hr = i4(/\*\*\*+/, "*"), Jr = i4(/([^/{[(!])\*\*/, (r, e) => `${e}*`), Qr = i4(/(^|.)\*\*(?=[^*/)\]}])/, (r, e) => `${e}*`), Wr = f(l([Tr, Hr, Jr, Qr, qr])), Kr = Wr, Xr = z(Kr), Yr = Xr, $ = (r, e) => {
...
var resolve2 = function(...arguments_) {
  ...
  let resolvedAbsolute = false;
  for (let index5 = arguments_.length - 1; index5 >= -1 && !resolvedAbsolute; index5--) {
    const path = index5 >= 0 ? arguments_[index5] : cwd2();
    if (!path || path.length === 0) {
      continue;
    }
    resolvedPath = `${path}/${resolvedPath}`;
...
var basename = function(p, extension) {
  ...
  let lastSegment = "";
  for (let i4 = segments.length - 1; i4 >= 0; i4--) {
    const val = segments[i4];
    if (val) {
      lastSegment = val;
      break;
    }
  ...
  usedFixtures.forEach((fixture) => {
    ...
    if (depSet.has(fixture)) {
      if (fixture.parent) {
        ...
        throw new Error(`Circular fixture dependency detected: ${fixture.name} <- ${[...depSet].reverse().map((d4) => d4.name).join(" <- ")}`);
      }
    }
    depSet.add(fixture);
...
function splitByComma(s) {
  ...
  for (let i4 = 0; i4 < s.length; i4++) {
    if (s[i4] === "{" || s[i4] === "[") {
      stack.push(s[i4] === "{" ? "}" : "]");
    } else if (s[i4] === stack.at(-1)) {
      stack.pop();
    } else if (!stack.length && s[i4] === ",") {
      const token = s.substring(start, i4).trim();
      if (token) {
        result.push(token);
      }
      start = i4 + 1;
...
function tokenize(expr) {
  ...
  while (i4 < expr.length) {
    ...
    if (/^and(?:\s|\)|$)/i.test(expr.slice(i4))) {
      tokens.push({ type: "AND" });
      i4 += 3;
      continue;
    }
    if (/^or(?:\s|\)|$)/i.test(expr.slice(i4))) {
      tokens.push({ type: "OR" });
      i4 += 2;
      continue;
    }
    if (/^not\s/i.test(expr.slice(i4))) {
      tokens.push({ type: "NOT" });
      i4 += 3;
      continue;
    }
    let tag = "";
    while (i4 < expr.length && expr[i4] !== " " && expr[i4] !== "	" && expr[i4] !== "(" && expr[i4] !== ")" && expr[i4] !== "!" && expr[i4] !== "&" && expr[i4] !== "|") {
      const remaining = expr.slice(i4);
...
function createSuiteCollector(name, factory = () => {
  ...
  const test7 = createTest(function(name2, optionsOrFn, timeoutOrTest) {
  ...
  async function collect(file) {
    ...
    const allChildren = [];
    for (const i4 of tasks) {
      allChildren.push(i4.type === "collector" ? await i4.collect(file) : i4);
    }
...
function createSuite() {
  ...
  suiteFn.each = function(cases, ...args) {
    ...
    return (name, optionsOrFn, fnOrOptions) => {
      ...
      cases.forEach((i4, idx) => {
        const items = Array.isArray(i4) ? i4 : [i4];
        if (fnFirst) {
          if (arrayOnlyCases) {
            suite2(formatTitle(_name, items, idx), handler ? () => handler(...items) : void 0, options.timeout);
          } else {
            suite2(formatTitle(_name, items, idx), handler ? () => handler(i4) : void 0, options.timeout);
          }
        } else {
          if (arrayOnlyCases) {
            suite2(formatTitle(_name, items, idx), options, handler ? () => handler(...items) : void 0);
          } else {
            suite2(formatTitle(_name, items, idx), options, handler ? () => handler(i4) : void 0);
          }
        }
      });
...
function createTaskCollector(fn2) {
  ...
  taskFn.each = function(cases, ...args) {
    const context = getChainableContext(this);
    const test7 = context.withContext();
    context.setContext("each", true);
    if (Array.isArray(cases) && args.length) {
      cases = formatTemplateString(cases, args);
    }
    return (name, optionsOrFn, fnOrOptions) => {
      ...
      cases.forEach((i4, idx) => {
        ...
        if (fnFirst) {
          if (arrayOnlyCases) {
            test7(formatTitle(_name, items, idx), handler ? () => handler(...items) : void 0, options.timeout);
          } else {
            test7(formatTitle(_name, items, idx), handler ? () => handler(i4) : void 0, options.timeout);
          }
        } else {
          if (arrayOnlyCases) {
            test7(formatTitle(_name, items, idx), options, handler ? () => handler(...items) : void 0);
          } else {
            test7(formatTitle(_name, items, idx), options, handler ? () => handler(i4) : void 0);
          }
        }
      });
      context.setContext("each", void 0);
  ...
  taskFn.for = function(cases, ...args) {
    ...
    return (name, optionsOrFn, fnOrOptions) => {
      ...
      cases.forEach((item, idx) => {
        ...
        test7(formatTitle(_name, toArray(item), idx), options, handlerWrapper);
      });
    };
  };
...
function formatTitle(template, items, idx) {
  ...
  if (template.includes("%f")) {
    ...
    placeholders.forEach((_, i5) => {
      if (isNegativeNaN(items[i5]) || Object.is(items[i5], -0)) {
        let occurrence = 0;
        template = template.replace(/%f/g, (match) => {
          occurrence++;
          return occurrence === i5 + 1 ? "-%f" : match;
        });
      }
    });
  }
  ...
  handleRegexMatch(
    ...
    (match) => {
      if (i4 < count) {
        output += format2(match[0], items[i4++]);
      } else {
        output += match[0];
      }
    },
...
function formatTemplateString(cases, args) {
  ...
  const res = [];
  for (let i4 = 0; i4 < Math.floor(args.length / header.length); i4++) {
    const oneCase = {};
    for (let j = 0; j < header.length; j++) {
      oneCase[header[j]] = args[i4 * header.length + j];
    }
    res.push(oneCase);
  }
  return res;
...
function createTestContext(test7, runner2) {
  ...
  context.annotate = (message, type3, attachment) => {
    ...
    return recordAsyncOperation(test7, recordArtifact(test7, {
      ...
      const resolvedAnnotation = await runner2.onTestAnnotate(test7, annotation2);
...
var table = [];
for (let i4 = 65; i4 < 91; i4++) {
  table.push(String.fromCharCode(i4));
}
for (let i4 = 97; i4 < 123; i4++) {
  table.push(String.fromCharCode(i4));
}
for (let i4 = 0; i4 < 10; i4++) {
  table.push(i4.toString(10));
}
table.push("+", "/");
function encodeUint8Array(bytes) {
  ...
  for (let i4 = 0; i4 < len; i4 += 3) {
    if (len === i4 + 1) {
      const a = (bytes[i4] & 252) >> 2;
      const b4 = (bytes[i4] & 3) << 4;
      base64 += table[a];
      base64 += table[b4];
      base64 += "==";
    } else if (len === i4 + 2) {
      const a = (bytes[i4] & 252) >> 2;
      const b4 = (bytes[i4] & 3) << 4 | (bytes[i4 + 1] & 240) >> 4;
      const c = (bytes[i4 + 1] & 15) << 2;
      base64 += table[a];
      base64 += table[b4];
      base64 += table[c];
      base64 += "=";
      ...
      const a = (bytes[i4] & 252) >> 2;
      const b4 = (bytes[i4] & 3) << 4 | (bytes[i4 + 1] & 240) >> 4;
      const c = (bytes[i4 + 1] & 15) << 2 | (bytes[i4 + 2] & 192) >> 6;
      const d4 = bytes[i4 + 2] & 63;
      base64 += table[a];
      base64 += table[b4];
      base64 += table[c];
      base64 += table[d4];
...
function recordAsyncOperation(test7, promise) {
  promise = promise.finally(() => {
    if (!test7.promises) {
      return;
    }
    const index5 = test7.promises.indexOf(promise);
    if (index5 !== -1) {
      test7.promises.splice(index5, 1);
    }
  });
  if (!test7.promises) {
    test7.promises = [];
...
// node_modules/vitest/node_modules/@vitest/utils/dist/constants.js
var KNOWN_ASSET_TYPES2 = [
  "apng",
  "bmp",
  "png",
  "jpe?g",
...
var KNOWN_ASSET_RE2 = new RegExp(`\\.(${KNOWN_ASSET_TYPES2.join("|")})$`);
...
function deepClone2(val, options = defaultCloneOptions) {
...
function clone(val, seen, options = defaultCloneOptions) {
  ...
  if (Object.prototype.toString.call(val) === "[object Object]") {
    ...
    seen.set(val, out);
    const props = getOwnProperties2(val);
...
// node_modules/vitest/node_modules/@vitest/utils/dist/timers.js
...
function getSafeTimers2() {
  ...
  const { nextTick: safeNextTick } = globalThis[SAFE_TIMERS_SYMBOL2] || globalThis.process || {};
  return {
    nextTick: safeNextTick,
    setTimeout: safeSetTimeout,
    setInterval: safeSetInterval,
...
function delay(timeout, scheduler = setTimeout) {
  return new Promise((resolve6) => scheduler(resolve6, timeout));
}
...
function waitNextTick() {
  const { setTimeout: setTimeout4 } = getSafeTimers2();
  return new Promise((resolve6) => setTimeout4(resolve6, 0));
}
...
// node_modules/vitest/node_modules/tinyrainbow/dist/index.js
var b2 = {
  reset: [0, 0],
  bold: [1, 22, "\x1B[22m\x1B[1m"],
  dim: [2, 22, "\x1B[22m\x1B[2m"],
...
function printIteratorEntries2(iterator, config2, indentation, depth, refs, printer4, separator = ": ") {
  let result = "";
  let width = 0;
  let current = iterator.next();
  if (!current.done) {
...
function printIteratorValues2(iterator, config2, indentation, depth, refs, printer4) {
  let result = "";
  let width = 0;
  let current = iterator.next();
  if (!current.done) {
...
function printListItems2(list, config2, indentation, depth, refs, printer4) {
  let result = "";
  list = list instanceof ArrayBuffer ? new DataView(list) : list;
  const isDataView = (l) => l instanceof DataView;
  const length = isDataView(list) ? list.byteLength : list.length;
...
function printObjectProperties2(val, config2, indentation, depth, refs, printer4) {
  let result = "";
...
var SPACE$22 = " ";
var serialize$52 = (val, config2, indentation, depth, refs, printer4) => {
  const stringedValue = val.toString();
  if (stringedValue === "ArrayContaining" || stringedValue === "ArrayNotContaining") {
    if (++depth > config2.maxDepth) {
      return `[${stringedValue}]`;
    }
    return `${stringedValue + SPACE$22}[${printListItems2(val.sample, config2, indentation, depth, refs, printer4)}]`;
  }
  if (stringedValue === "ObjectContaining" || stringedValue === "ObjectNotContaining") {
    if (++depth > config2.maxDepth) {
      return `[${stringedValue}]`;
    }
    return `${stringedValue + SPACE$22}{${printObjectProperties2(val.sample, config2, indentation, depth, refs, printer4)}}`;
  }
  if (stringedValue === "StringMatching" || stringedValue === "StringNotMatching") {
    return stringedValue + SPACE$22 + printer4(val.sample, config2, indentation, depth, refs);
  }
  if (stringedValue === "StringContaining" || stringedValue === "StringNotContaining") {
    return stringedValue + SPACE$22 + printer4(val.sample, config2, indentation, depth, refs);
  }
...
var serialize$42 = (collection, config2, indentation, depth, refs, printer4) => {
  const name = collection.constructor.name;
  if (++depth > config2.maxDepth) {
    return `[${name}]`;
  }
  return (config2.min ? "" : name + SPACE$12) + (OBJECT_NAMES2.has(name) ? `{${printObjectProperties2(isNamedNodeMap2(collection) ? [...collection].reduce((props, attribute) => {
    props[attribute.name] = attribute.value;
    return props;
  }, {}) : { ...collection }, config2, indentation, depth, refs, printer4)}}` : `[${printListItems2([...collection], config2, indentation, depth, refs, printer4)}]`);
};
var plugin$42 = {
...
function printProps2(keys2, props, config2, indentation, depth, refs, printer4) {
  const indentationNext = indentation + config2.indent;
  const colors = config2.colors;
  return keys2.map((key) => {
    const value = props[key];
...
function printChildren2(children, config2, indentation, depth, refs, printer4) {
  return children.map((child) => config2.spacingOuter + indentation + (typeof child === "string" ? printText2(child, config2) : printer4(child, config2, indentation, depth, refs))).join("");
}
function printShadowRoot2(children, config2, indentation, depth, refs, printer4) {
  if (config2.printShadowRoot === false) {
    return "";
  }
  return [`${config2.spacingOuter + indentation}#shadow-root`, printChildren2(children, config2, indentation + config2.indent, depth, refs, printer4)].join("");
}
...
function filterChildren2(children, filterNode) {
  let filtered = children.filter((node) => {
...
function serializeDOM2(node, config2, indentation, depth, refs, printer4, filterNode) {
  if (nodeIsText2(node)) {
  ...
  const children = Array.prototype.slice.call(node.childNodes || node.children);
  const shadowChildren = nodeIsFragment2(node) || !node.shadowRoot ? [] : Array.prototype.slice.call(node.shadowRoot.children);
  const resolvedChildren = filterNode ? filterChildren2(children, filterNode) : children;
  const resolvedShadowChildren = filterNode ? filterChildren2(shadowChildren, filterNode) : shadowChildren;
  return printElement2(type3, printProps2(nodeIsFragment2(node) ? [] : Array.from(node.attributes, (attr) => attr.name).sort(), nodeIsFragment2(node) ? {} : [...node.attributes].reduce((props, attribute) => {
    props[attribute.name] = attribute.value;
    return props;
  }, {}), config2, indentation + config2.indent, depth, refs, printer4), (resolvedShadowChildren.length > 0 ? printShadowRoot2(resolvedShadowChildren, config2, indentation + config2.indent, depth, refs, printer4) : "") + printChildren2(resolvedChildren, config2, indentation + config2.indent, depth, refs, printer4), config2, indentation);
}
var serialize$32 = (node, config2, indentation, depth, refs, printer4) => serializeDOM2(node, config2, indentation, depth, refs, printer4);
function createDOMElementFilter2(filterNode) {
  return {
    test: test$32,
    serialize: (node, config2, indentation, depth, refs, printer4) => serializeDOM2(node, config2, indentation, depth, refs, printer4, filterNode)
  };
}
var plugin$32 = {
...
var SPACE2 = " ";
var LAZY2 = "…";
function printImmutableEntries2(val, config2, indentation, depth, refs, printer4, type3) {
  return ++depth > config2.maxDepth ? printAsLeaf2(getImmutableName2(type3)) : `${getImmutableName2(type3) + SPACE2}{${printIteratorEntries2(val.entries(), config2, indentation, depth, refs, printer4)}}`;
}
...
function printImmutableRecord2(val, config2, indentation, depth, refs, printer4) {
  const name = getImmutableName2(val._name || "Record");
  return ++depth > config2.maxDepth ? printAsLeaf2(name) : `${name + SPACE2}{${printIteratorEntries2(getRecordEntries2(val), config2, indentation, depth, refs, printer4)}}`;
}
function printImmutableSeq2(val, config2, indentation, depth, refs, printer4) {
  const name = getImmutableName2("Seq");
  if (++depth > config2.maxDepth) {
    return printAsLeaf2(name);
  }
  if (val[IS_KEYED_SENTINEL2]) {
    return `${name + SPACE2}{${val._iter || val._object ? printIteratorEntries2(val.entries(), config2, indentation, depth, refs, printer4) : LAZY2}}`;
  }
  return `${name + SPACE2}[${val._iter || val._array || val._collection || val._iterable ? printIteratorValues2(val.values(), config2, indentation, depth, refs, printer4) : LAZY2}]`;
}
function printImmutableValues2(val, config2, indentation, depth, refs, printer4, type3) {
  return ++depth > config2.maxDepth ? printAsLeaf2(getImmutableName2(type3)) : `${getImmutableName2(type3) + SPACE2}[${printIteratorValues2(val.values(), config2, indentation, depth, refs, printer4)}]`;
}
var serialize$22 = (val, config2, indentation, depth, refs, printer4) => {
  if (val[IS_MAP_SENTINEL2]) {
    return printImmutableEntries2(val, config2, indentation, depth, refs, printer4, val[IS_ORDERED_SENTINEL2] ? "OrderedMap" : "Map");
  }
  if (val[IS_LIST_SENTINEL2]) {
    return printImmutableValues2(val, config2, indentation, depth, refs, printer4, "List");
  }
  if (val[IS_SET_SENTINEL2]) {
    return printImmutableValues2(val, config2, indentation, depth, refs, printer4, val[IS_ORDERED_SENTINEL2] ? "OrderedSet" : "Set");
  }
  if (val[IS_STACK_SENTINEL2]) {
    return printImmutableValues2(val, config2, indentation, depth, refs, printer4, "Stack");
  }
  if (val[IS_SEQ_SENTINEL2]) {
    return printImmutableSeq2(val, config2, indentation, depth, refs, printer4);
  }
  return printImmutableRecord2(val, config2, indentation, depth, refs, printer4);
};
...
var reactIs_production2 = {};
var hasRequiredReactIs_production2;
function requireReactIs_production2() {
  if (hasRequiredReactIs_production2) return reactIs_production2;
  hasRequiredReactIs_production2 = 1;
  var REACT_ELEMENT_TYPE = Symbol.for("react.transitional.element"), REACT_PORTAL_TYPE = Symbol.for("react.portal"), REACT_FRAGMENT_TYPE = Symbol.for("react.fragment"), REACT_STRICT_MODE_TYPE = Symbol.for("react.strict_mode"), REACT_PROFILER_TYPE = Symbol.for("react.profiler"), REACT_CONSUMER_TYPE = Symbol.for("react.consumer"), REACT_CONTEXT_TYPE = Symbol.for("react.context"), REACT_FORWARD_REF_TYPE = Symbol.for("react.forward_ref"), REACT_SUSPENSE_TYPE = Symbol.for("react.suspense"), REACT_SUSPENSE_LIST_TYPE = Symbol.for("react.suspense_list"), REACT_MEMO_TYPE = Symbol.for("react.memo"), REACT_LAZY_TYPE = Symbol.for("react.lazy"), REACT_VIEW_TRANSITION_TYPE = Symbol.for("react.view_transition"), REACT_CLIENT_REFERENCE = Symbol.for("react.client.reference");
  function typeOf2(object2) {
    if ("object" === typeof object2 && null !== object2) {
      var $$typeof = object2.$$typeof;
  ...
  reactIs_production2.ContextConsumer = REACT_CONSUMER_TYPE;
  reactIs_production2.ContextProvider = REACT_CONTEXT_TYPE;
  reactIs_production2.Element = REACT_ELEMENT_TYPE;
  reactIs_production2.ForwardRef = REACT_FORWARD_REF_TYPE;
  reactIs_production2.Fragment = REACT_FRAGMENT_TYPE;
  reactIs_production2.Lazy = REACT_LAZY_TYPE;
  reactIs_production2.Memo = REACT_MEMO_TYPE;
  reactIs_production2.Portal = REACT_PORTAL_TYPE;
  reactIs_production2.Profiler = REACT_PROFILER_TYPE;
  reactIs_production2.StrictMode = REACT_STRICT_MODE_TYPE;
  reactIs_production2.Suspense = REACT_SUSPENSE_TYPE;
  reactIs_production2.SuspenseList = REACT_SUSPENSE_LIST_TYPE;
  reactIs_production2.isContextConsumer = function(object2) {
    return typeOf2(object2) === REACT_CONSUMER_TYPE;
  };
  reactIs_production2.isContextProvider = function(object2) {
    return typeOf2(object2) === REACT_CONTEXT_TYPE;
  };
  reactIs_production2.isElement = function(object2) {
    return "object" === typeof object2 && null !== object2 && object2.$$typeof === REACT_ELEMENT_TYPE;
  };
  reactIs_production2.isForwardRef = function(object2) {
    return typeOf2(object2) === REACT_FORWARD_REF_TYPE;
  };
  reactIs_production2.isFragment = function(object2) {
    return typeOf2(object2) === REACT_FRAGMENT_TYPE;
  };
  reactIs_production2.isLazy = function(object2) {
    return typeOf2(object2) === REACT_LAZY_TYPE;
  };
  reactIs_production2.isMemo = function(object2) {
    return typeOf2(object2) === REACT_MEMO_TYPE;
  };
  reactIs_production2.isPortal = function(object2) {
    return typeOf2(object2) === REACT_PORTAL_TYPE;
  };
  reactIs_production2.isProfiler = function(object2) {
    return typeOf2(object2) === REACT_PROFILER_TYPE;
  };
  reactIs_production2.isStrictMode = function(object2) {
    return typeOf2(object2) === REACT_STRICT_MODE_TYPE;
  };
  reactIs_production2.isSuspense = function(object2) {
    return typeOf2(object2) === REACT_SUSPENSE_TYPE;
  };
  reactIs_production2.isSuspenseList = function(object2) {
    return typeOf2(object2) === REACT_SUSPENSE_LIST_TYPE;
  };
  reactIs_production2.isValidElementType = function(type3) {
    return "string" === typeof type3 || "function" === typeof type3 || type3 === REACT_FRAGMENT_TYPE || type3 === REACT_PROFILER_TYPE || type3 === REACT_STRICT_MODE_TYPE || type3 === REACT_SUSPENSE_TYPE || type3 === REACT_SUSPENSE_LIST_TYPE || "object" === typeof type3 && null !== type3 && (type3.$$typeof === REACT_LAZY_TYPE || type3.$$typeof === REACT_MEMO_TYPE || type3.$$typeof === REACT_CONTEXT_TYPE || type3.$$typeof === REACT_CONSUMER_TYPE || type3.$$typeof === REACT_FORWARD_REF_TYPE || type3.$$typeof === REACT_CLIENT_REFERENCE || void 0 !== type3.getModuleId) ? true : false;
  };
  reactIs_production2.typeOf = typeOf2;
  return reactIs_production2;
}
...
function requireReactIs$12() {
  ...
  {
    reactIs$12.exports = requireReactIs_production2();
  }
...
var hasRequiredReactIs_production_min2;
function requireReactIs_production_min2() {
  if (hasRequiredReactIs_production_min2) return reactIs_production_min2;
  hasRequiredReactIs_production_min2 = 1;
  var b4 = Symbol.for("react.element"), c = Symbol.for("react.portal"), d4 = Symbol.for("react.fragment"), e = Symbol.for("react.strict_mode"), f = Symbol.for("react.profiler"), g = Symbol.for("react.provider"), h = Symbol.for("react.context"), k = Symbol.for("react.server_context"), l = Symbol.for("react.forward_ref"), m = Symbol.for("react.suspense"), n = Symbol.for("react.suspense_list"), p = Symbol.for("react.memo"), q = Symbol.for("react.lazy"), t = Symbol.for("react.offscreen"), u;
  u = Symbol.for("react.module.reference");
  function v(a) {
    if ("object" === typeof a && null !== a) {
  ...
  reactIs_production_min2.ContextConsumer = h;
  reactIs_production_min2.ContextProvider = g;
  reactIs_production_min2.Element = b4;
  reactIs_production_min2.ForwardRef = l;
  reactIs_production_min2.Fragment = d4;
  reactIs_production_min2.Lazy = q;
  reactIs_production_min2.Memo = p;
  reactIs_production_min2.Portal = c;
  reactIs_production_min2.Profiler = f;
  reactIs_production_min2.StrictMode = e;
  reactIs_production_min2.Suspense = m;
  reactIs_production_min2.SuspenseList = n;
  reactIs_production_min2.isAsyncMode = function() {
    return false;
  };
  reactIs_production_min2.isConcurrentMode = function() {
    return false;
  };
  reactIs_production_min2.isContextConsumer = function(a) {
    return v(a) === h;
  };
  reactIs_production_min2.isContextProvider = function(a) {
    return v(a) === g;
  };
  reactIs_production_min2.isElement = function(a) {
    return "object" === typeof a && null !== a && a.$$typeof === b4;
  };
  reactIs_production_min2.isForwardRef = function(a) {
    return v(a) === l;
  };
  reactIs_production_min2.isFragment = function(a) {
    return v(a) === d4;
  };
  reactIs_production_min2.isLazy = function(a) {
    return v(a) === q;
  };
  reactIs_production_min2.isMemo = function(a) {
    return v(a) === p;
  };
  reactIs_production_min2.isPortal = function(a) {
    return v(a) === c;
  };
  reactIs_production_min2.isProfiler = function(a) {
    return v(a) === f;
  };
  reactIs_production_min2.isStrictMode = function(a) {
    return v(a) === e;
  };
  reactIs_production_min2.isSuspense = function(a) {
    return v(a) === m;
  };
  reactIs_production_min2.isSuspenseList = function(a) {
    return v(a) === n;
  };
  reactIs_production_min2.isValidElementType = function(a) {
    return "string" === typeof a || "function" === typeof a || a === d4 || a === f || a === e || a === m || a === n || a === t || "object" === typeof a && null !== a && (a.$$typeof === q || a.$$typeof === p || a.$$typeof === g || a.$$typeof === h || a.$$typeof === l || a.$$typeof === u || void 0 !== a.getModuleId) ? true : false;
  };
  reactIs_production_min2.typeOf = v;
  return reactIs_production_min2;
}
...
function requireReactIs2() {
  ...
  {
    reactIs2.exports = requireReactIs_production_min2();
  }
...
var ReactIs2 = Object.fromEntries(reactIsMethods2.map((m) => [m, (v) => ReactIs182[m](v) || ReactIs192[m](v)]));
...
var serialize$12 = (element, config2, indentation, depth, refs, printer4) => ++depth > config2.maxDepth ? printElementAsLeaf2(getType4(element), config2) : printElement2(getType4(element), printProps2(getPropKeys$12(element), element.props, config2, indentation + config2.indent, depth, refs, printer4), printChildren2(getChildren2(element.props.children), config2, indentation + config2.indent, depth, refs, printer4), config2, indentation);
...
var serialize2 = (object2, config2, indentation, depth, refs, printer4) => ++depth > config2.maxDepth ? printElementAsLeaf2(object2.type, config2) : printElement2(object2.type, object2.props ? printProps2(getPropKeys2(object2), object2.props, config2, indentation + config2.indent, depth, refs, printer4) : "", object2.children ? printChildren2(object2.children, config2, indentation + config2.indent, depth, refs, printer4) : "", config2, indentation);
...
function printBasicValue2(val, printFunctionName2, escapeRegex2, escapeString) {
  ...
  if (toStringed === "[object Date]") {
    return Number.isNaN(+val) ? "Date { NaN }" : toISOString2.call(val);
  }
  if (toStringed === "[object Error]") {
  ...
  if (toStringed === "[object RegExp]") {
    if (escapeRegex2) {
      return regExpToString2.call(val).replaceAll(/[$()*+.?[\\\]^{|}]/g, "\\$&");
    }
...
function printComplexValue2(val, config2, indentation, depth, refs, hasCalledToJSON) {
  ...
  const toStringed = toString3.call(val);
  if (toStringed === "[object Arguments]") {
    return hitMaxDepth ? "[Arguments]" : `${min ? "" : "Arguments "}[${printListItems2(val, config2, indentation, depth, refs, printer2)}]`;
  }
  if (isToStringedArrayType2(toStringed)) {
    return hitMaxDepth ? `[${val.constructor.name}]` : `${min ? "" : !config2.printBasicPrototype && val.constructor.name === "Array" ? "" : `${val.constructor.name} `}[${printListItems2(val, config2, indentation, depth, refs, printer2)}]`;
  }
  if (toStringed === "[object Map]") {
    return hitMaxDepth ? "[Map]" : `Map {${printIteratorEntries2(val.entries(), config2, indentation, depth, refs, printer2, " => ")}}`;
  }
  if (toStringed === "[object Set]") {
    return hitMaxDepth ? "[Set]" : `Set {${printIteratorValues2(val.values(), config2, indentation, depth, refs, printer2)}}`;
  }
  return hitMaxDepth || isWindow2(val) ? `[${getConstructorName2(val)}]` : `${min ? "" : !config2.printBasicPrototype && getConstructorName2(val) === "Object" ? "" : `${getConstructorName2(val)} `}{${printObjectProperties2(val, config2, indentation, depth, refs, printer2)}}`;
}
var ErrorPlugin2 = {
  test: (val) => val && val instanceof Error,
  serialize(val, config2, indentation, depth, refs, printer4) {
    ...
    const name = val.name !== "Error" ? val.name : getConstructorName2(val);
    return hitMaxDepth ? `[${name}]` : `${name} {${printIteratorEntries2(Object.entries(entries).values(), config2, indentation, depth, refs, printer4)}}`;
  }
};
...
function printer2(val, config2, indentation, depth, refs, hasCalledToJSON) {
  ...
  if (plugin5 !== null) {
    result = printPlugin2(plugin5, val, config2, indentation, depth, refs);
  } else {
    const basicResult = printBasicValue2(val, config2.printFunctionName, config2.escapeRegex, config2.escapeString);
    if (basicResult !== null) {
      result = basicResult;
    } else {
      result = printComplexValue2(val, config2, indentation, depth, refs, hasCalledToJSON);
    }
  }
  (_a = config2._outputLengthPerDepth)[depth] ?? (_a[depth] = 0);
  config2._outputLengthPerDepth[depth] += result.length;
...
function validateOptions2(options) {
  for (const key of Object.keys(options)) {
    if (!Object.hasOwn(DEFAULT_OPTIONS2, key)) {
      throw new Error(`pretty-format: Unknown option "${key}".`);
    }
  }
  if (options.min && options.indent !== void 0 && options.indent !== 0) {
...
function getColorsHighlight2() {
  return DEFAULT_THEME_KEYS2.reduce((colors, key) => {
    const value = DEFAULT_THEME2[key];
    const color = value && y2[value];
    if (color && typeof color.close === "string" && typeof color.open === "string") {
      colors[key] = color;
    } else {
      throw new Error(`pretty-format: Option "theme" has a key "${key}" whose value "${value}" is undefined in ansi-styles.`);
...
function getConfig2(options) {
  return {
    callToJSON: (options == null ? void 0 : options.callToJSON) ?? DEFAULT_OPTIONS2.callToJSON,
    colors: (options == null ? void 0 : options.highlight) ? getColorsHighlight2() : getColorsEmpty2(),
    compareKeys: typeof (options == null ? void 0 : options.compareKeys) === "function" || (options == null ? void 0 : options.compareKeys) === null ? options.compareKeys : DEFAULT_OPTIONS2.compareKeys,
    escapeRegex: getEscapeRegex2(options),
    escapeString: getEscapeString2(options),
    indent: (options == null ? void 0 : options.min) ? "" : createIndent2((options == null ? void 0 : options.indent) ?? DEFAULT_OPTIONS2.indent),
    maxDepth: (options == null ? void 0 : options.maxDepth) ?? DEFAULT_OPTIONS2.maxDepth,
    maxWidth: (options == null ? void 0 : options.maxWidth) ?? DEFAULT_OPTIONS2.maxWidth,
    min: (options == null ? void 0 : options.min) ?? DEFAULT_OPTIONS2.min,
    plugins: (options == null ? void 0 : options.plugins) ?? DEFAULT_OPTIONS2.plugins,
    printBasicPrototype: (options == null ? void 0 : options.printBasicPrototype) ?? true,
    printFunctionName: getPrintFunctionName2(options),
    printShadowRoot: (options == null ? void 0 : options.printShadowRoot) ?? true,
    spacingInner: (options == null ? void 0 : options.min) ? " " : "\n",
    spacingOuter: (options == null ? void 0 : options.min) ? "" : "\n",
    maxOutputLength: (options == null ? void 0 : options.maxOutputLength) ?? DEFAULT_OPTIONS2.maxOutputLength,
    _outputLengthPerDepth: []
  };
}
...
// node_modules/vitest/node_modules/@vitest/utils/dist/display.js
...
function stringify2(object2, maxDepth = 10, { maxLength, filterNode, ...options } = {}) {
  const MAX_LENGTH = maxLength ?? 1e4;
  let result;
  const filterFn = typeof filterNode === "string" ? createNodeFilterFromSelector2(filterNode) : filterNode;
  const plugins4 = filterFn ? [
    ReactTestComponent3,
    ReactElement3,
    createDOMElementFilter2(filterFn),
    DOMCollection3,
    Immutable3,
    AsymmetricMatcher3
  ] : PLUGINS3;
  try {
    result = format4(object2, {
      maxDepth,
      escapeString: false,
      plugins: plugins4,
      ...options
...
function createNodeFilterFromSelector2(selector) {
  const ELEMENT_NODE4 = 1;
  const COMMENT_NODE4 = 8;
  return (node) => {
    if (node.nodeType === COMMENT_NODE4) {
...
function joinAlignedDiffsNoExpand(diffs, options) {
  ...
  while (i4 !== iLength) {
    const iStart = i4;
    while (i4 !== iLength && diffs[i4][0] === DIFF_EQUAL) {
      i4 += 1;
    }
    if (iStart !== i4) {
      if (iStart === 0) {
        if (i4 > nContextLines) {
          jLength -= i4 - nContextLines;
  ...
  while (i4 !== iLength) {
    ...
    if (iStart !== i4) {
      if (iStart === 0) {
        if (i4 > nContextLines) {
          iStart = i4 - nContextLines;
          aStart = iStart;
          bStart = iStart;
          aEnd = aStart;
          bEnd = bStart;
        }
        for (let iCommon = iStart; iCommon !== i4; iCommon += 1) {
          pushCommonLine(diffs[iCommon][1]);
        }
      } else if (i4 === iLength) {
        const iEnd = i4 - iStart > nContextLines ? iStart + nContextLines : i4;
        for (let iCommon = iStart; iCommon !== iEnd; iCommon += 1) {
          pushCommonLine(diffs[iCommon][1]);
        }
      } else {
        const nCommon = i4 - iStart;
        if (nCommon > nContextLines2) {
          ...
          aEnd = aStart;
          bEnd = bStart;
          for (let iCommon = i4 - nContextLines; iCommon !== i4; iCommon += 1) {
            pushCommonLine(diffs[iCommon][1]);
          }
        } else {
          for (let iCommon = iStart; iCommon !== i4; iCommon += 1) {
    ...
    while (i4 !== iLength && diffs[i4][0] === DIFF_DELETE) {
      pushDeleteLine(diffs[i4][1]);
      i4 += 1;
    }
    while (i4 !== iLength && diffs[i4][0] === DIFF_INSERT) {
      pushInsertLine(diffs[i4][1]);
      i4 += 1;
    }
  }
  if (hasPatch) {
...
function joinAlignedDiffsExpand(diffs, options) {
  return diffs.map((diff2, i4, diffs2) => {
    const line = diff2[1];
    const isFirstOrLast = i4 === 0 || i4 === diffs2.length - 1;
    switch (diff2[0]) {
      case DIFF_DELETE:
...
function printAnnotation({ aAnnotation, aColor, aIndicator, bAnnotation, bColor, bIndicator, includeChangeCounts, omitAnnotationLines }, changeCounts) {
  ...
  const b4 = `${bIndicator} ${bAnnotation}${bRest}`;
  return `${aColor(a)}
${bColor(b4)}
...
function diffLinesRaw(aLines, bLines, options) {
  ...
  diffSequences2(aLength, bLength, isCommon, foundSubsequence);
...
function diffStrings(a, b4, options) {
  const truncate3 = (options == null ? void 0 : options.truncateThreshold) ?? false;
  const truncateThreshold = Math.max(Math.floor((options == null ? void 0 : options.truncateThreshold) ?? 0), 0);
  let aLength = a.length;
  let bLength = b4.length;
  if (truncate3) {
    const aMultipleLines = a.includes("\n");
    const bMultipleLines = b4.includes("\n");
    const aNewLineSymbol = getNewLineSymbol(a);
    const bNewLineSymbol = getNewLineSymbol(b4);
    const _a = aMultipleLines ? `${a.split(aNewLineSymbol, truncateThreshold).join(aNewLineSymbol)}
` : a;
    const _b = bMultipleLines ? `${b4.split(bNewLineSymbol, truncateThreshold).join(bNewLineSymbol)}
` : b4;
    aLength = _a.length;
    bLength = _b.length;
  }
  const truncated = aLength !== a.length || bLength !== b4.length;
  const isCommon = (aIndex2, bIndex2) => a[aIndex2] === b4[bIndex2];
  let aIndex = 0;
  let bIndex = 0;
  const diffs = [];
  const foundSubsequence = (nCommon, aCommon, bCommon) => {
    ...
    if (bIndex !== bCommon) {
      diffs.push(new Diff(DIFF_INSERT, b4.slice(bIndex, bCommon)));
    }
    aIndex = aCommon + nCommon;
    bIndex = bCommon + nCommon;
    diffs.push(new Diff(DIFF_EQUAL, b4.slice(bCommon, bIndex)));
  };
  diffSequences2(aLength, bLength, isCommon, foundSubsequence);
  if (aIndex !== aLength) {
  ...
  if (bIndex !== bLength) {
    diffs.push(new Diff(DIFF_INSERT, b4.slice(bIndex)));
  }
  return [diffs, truncated];
}
...
function hasCommonDiff(diffs, isMultiline) {
  if (isMultiline) {
    ...
    return diffs.some((diff2, i4) => diff2[0] === DIFF_EQUAL && (i4 !== iLast || diff2[1] !== "\n"));
  }
  return diffs.some((diff2) => diff2[0] === DIFF_EQUAL);
}
function diffStringsUnified(a, b4, options) {
  if (a !== b4 && a.length !== 0 && b4.length !== 0) {
...
function diffStringsRaw(a, b4, cleanup, options) {
  const [diffs, truncated] = diffStrings(a, b4, options);
  if (cleanup) {
...
var { AsymmetricMatcher: AsymmetricMatcher4, DOMCollection: DOMCollection4, DOMElement: DOMElement4, Immutable: Immutable4, ReactElement: ReactElement4, ReactTestComponent: ReactTestComponent4 } = plugins2;
var PLUGINS4 = [
  ReactTestComponent4,
  ReactElement4,
  DOMElement4,
...
function diff(a, b4, options) {
  if (Object.is(a, b4)) {
    return "";
  }
  const aType = getType5(a);
  ...
  if (expectedType !== getType5(b4)) {
    let truncate3 = function(s) {
      return s.length <= MAX_LENGTH ? s : `${s.slice(0, MAX_LENGTH)}...`;
    };
    const { aAnnotation, aColor, aIndicator, bAnnotation, bColor, bIndicator } = normalizeDiffOptions(options);
    const formatOptions = getFormatOptions(FALLBACK_FORMAT_OPTIONS, options);
    let aDisplay = format4(a, formatOptions);
    let bDisplay = format4(b4, formatOptions);
...
${bDiff}`;
  ...
  switch (aType) {
    ...
    case "boolean":
    case "number":
      return comparePrimitive(a, b4, options);
    case "map":
      return compareObjects(sortMap(a), sortMap(b4), options);
    case "set":
      return compareObjects(sortSet(a), sortSet(b4), options);
    default:
      return compareObjects(a, b4, options);
  }
}
function comparePrimitive(a, b4, options) {
  const aFormat = format4(a, FORMAT_OPTIONS);
  const bFormat = format4(b4, FORMAT_OPTIONS);
...
function compareObjects(a, b4, options) {
  let difference;
  let hasThrown = false;
  try {
    const formatOptions = getFormatOptions(FORMAT_OPTIONS, options);
    difference = getObjectsDifference(a, b4, formatOptions, options);
  } catch {
    hasThrown = true;
  }
...
function getObjectsDifference(a, b4, formatOptions, options) {
  const formatOptionsZeroIndent = {
    ...formatOptions,
    indent: 0
  };
  const aCompare = format4(a, formatOptionsZeroIndent);
  const bCompare = format4(b4, formatOptionsZeroIndent);
  if (aCompare === bCompare) {
    return getCommonMessage(NO_DIFF_MESSAGE, options);
  } else {
    const aDisplay = format4(a, formatOptions);
    const bDisplay = format4(b4, formatOptions);
    return diffLinesUnified2(aDisplay.split("\n"), bDisplay.split("\n"), aCompare.split("\n"), bCompare.split("\n"), options);
  }
}
var MAX_DIFF_STRING_LENGTH = 2e4;
function isAsymmetricMatcher(data) {
  const type3 = getType3(data);
  return type3 === "Object" && typeof data.asymmetricMatch === "function";
}
function isReplaceable(obj1, obj2) {
  const obj1Type = getType3(obj1);
  const obj2Type = getType3(obj2);
  return obj1Type === obj2Type && (obj1Type === "Object" || obj1Type === "Array");
}
function printDiffOrStringify2(received, expected, options) {
...
${receivedLine}`;
  ...
  const clonedExpected = deepClone2(expected, { forceWritable: true });
  const clonedReceived = deepClone2(received, { forceWritable: true });
...
function replaceAsymmetricMatcher(actual, expected, actualReplaced = /* @__PURE__ */ new WeakSet(), expectedReplaced = /* @__PURE__ */ new WeakSet()) {
  ...
  getOwnProperties2(expected).forEach((key) => {
...
function printReceived(object2) {
  return y2.red(replaceTrailingSpaces(stringify2(object2)));
}
function printExpected(value) {
  return y2.green(replaceTrailingSpaces(stringify2(value)));
}
function getCommonAndChangedSubstrings(diffs, op, hasCommonDiff2) {
  return diffs.reduce((reduced, diff2) => reduced + (diff2[0] === DIFF_EQUAL ? diff2[1] : diff2[0] === op ? hasCommonDiff2 ? y2.inverse(diff2[1]) : diff2[1] : ""), "");
}
...
// node_modules/vitest/node_modules/@vitest/utils/dist/serialize.js
...
function serializeValue2(val, seen = /* @__PURE__ */ new WeakMap()) {
  ...
  if (val instanceof Error && "toJSON" in val && typeof val.toJSON === "function") {
    ...
    if (jsonValue && jsonValue !== val && typeof jsonValue === "object") {
      ...
      if (val.cause != null) {
        safe(() => jsonValue.cause ?? (jsonValue.cause = serializeValue2(val.cause, seen)));
      }
    }
    return serializeValue2(jsonValue, seen);
  }
  ...
  if (isImmutable(val)) {
    return serializeValue2(val.toJSON(), seen);
  }
  ...
  if (Array.isArray(val)) {
    ...
    val.forEach((e, i4) => {
      try {
        clone2[i4] = serializeValue2(e, seen);
      } catch (err) {
        clone2[i4] = getUnserializableMessage(err);
      }
    });
    return clone2;
  } else {
    ...
    seen.set(val, clone2);
    let obj = val;
    while (obj && obj !== OBJECT_PROTO2) {
      Object.getOwnPropertyNames(obj).forEach((key) => {
        if (key in clone2) {
          return;
        }
        try {
          clone2[key] = serializeValue2(val[key], seen);
        } catch (err) {
          delete clone2[key];
...
function processError2(_err, diffOptions, seen = /* @__PURE__ */ new WeakSet()) {
  ...
  if (err.showDiff || err.showDiff === void 0 && err.expected !== void 0 && err.actual !== void 0) {
    err.diff = printDiffOrStringify2(err.actual, err.expected, {
      ...diffOptions,
      ...err.diffOptions
    });
  }
  ...
  try {
    if (!seen.has(err) && typeof err.cause === "object") {
      seen.add(err);
      err.cause = processError2(err.cause, diffOptions, seen);
    }
  } catch {
  }
  try {
...
__export2(check_error_exports, {
  ...
  getConstructorName: () => getConstructorName3,
...
function getConstructorName3(errorLike) {
...
__name(getConstructorName3, "getConstructorName");
...
function expectTypes(obj, types) {
  ...
  types.sort();
  let str = types.map(function(t, index5) {
    let art = ~["a", "e", "i", "o", "u"].indexOf(t.charAt(0)) ? "an" : "a";
    let or = types.length > 1 && index5 === types.length - 1 ? "or " : "";
    return or + art + " " + t;
  }).join(", ");
...
function inspectTypedArray2(array2, options) {
  ...
  let output = "";
  for (let i4 = 0; i4 < array2.length; i4++) {
    const string2 = `${options.stylize(truncate2(array2[i4], options.truncate), "number")}${i4 === array2.length - 1 ? "" : ", "}`;
    options.truncate -= string2.length;
    if (array2[i4] !== array2.length && options.truncate <= 3) {
      output += `${truncator2}(${array2.length - array2[i4] + 1})`;
      break;
    }
    output += string2;
  }
...
var stringEscapeChars3 = new RegExp("['\\u0000-\\u001f\\u007f-\\u009f\\u00ad\\u0600-\\u0604\\u070f\\u17b4\\u17b5\\u200c-\\u200f\\u2028-\\u202f\\u2060-\\u206f\\ufeff\\ufff0-\\uffff]", "g");
...
function inspectString2(string2, options) {
  if (stringEscapeChars3.test(string2)) {
    string2 = string2.replace(stringEscapeChars3, escape2);
  }
...
var symbolsSupported3 = typeof Symbol === "function" && typeof Symbol.for === "function";
var chaiInspect3 = symbolsSupported3 ? Symbol.for("chai/inspect") : "@@chai/inspect";
var nodeInspect3 = Symbol.for("nodejs.util.inspect.custom");
...
var inspectCustom2 = __name((value, options, type3, inspectFn) => {
  if (chaiInspect3 in value && typeof value[chaiInspect3] === "function") {
    return value[chaiInspect3](options);
  }
  if (nodeInspect3 in value && typeof value[nodeInspect3] === "function") {
    return value[nodeInspect3](options.depth, options, inspectFn);
  }
  if ("inspect" in value && typeof value.inspect === "function") {
    return value.inspect(options.depth, options);
  }
...
function iterableEqual(leftHandOperand, rightHandOperand, options) {
  ...
  var index5 = -1;
  while (++index5 < length) {
    if (deepEqual(leftHandOperand[index5], rightHandOperand[index5], options) === false) {
      return false;
    }
  }
  return true;
...
function getEnumerableSymbols(target) {
  ...
  for (var i4 = 0; i4 < allKeys.length; i4 += 1) {
    var key = allKeys[i4];
...
function keysEqual(leftHandOperand, rightHandOperand, keys2, options) {
  ...
  for (var i4 = 0; i4 < length; i4 += 1) {
    if (deepEqual(leftHandOperand[keys2[i4]], rightHandOperand[keys2[i4]], options) === false) {
      return false;
    }
  }
  return true;
...
function stringDistanceCapped(strA, strB, cap) {
  ...
  let memo2 = [];
  for (let i4 = 0; i4 <= strA.length; i4++) {
    memo2[i4] = Array(strB.length + 1).fill(0);
    memo2[i4][0] = i4;
  }
  for (let j = 0; j < strB.length; j++) {
    memo2[0][j] = j;
  }
  for (let i4 = 1; i4 <= strA.length; i4++) {
    let ch = strA.charCodeAt(i4 - 1);
    for (let j = 1; j <= strB.length; j++) {
      if (Math.abs(i4 - j) >= cap) {
        memo2[i4][j] = cap;
        continue;
      }
      memo2[i4][j] = Math.min(
        memo2[i4 - 1][j] + 1,
        memo2[i4][j - 1] + 1,
        memo2[i4 - 1][j - 1] + (ch === strB.charCodeAt(j - 1) ? 0 : 1)
      );
    }
  }
...
function compareByInspect(a, b4) {
  return inspect22(a) < inspect22(b4) ? -1 : 1;
}
...
function SameValueZero(a, b4) {
  return isNaN22(a) && isNaN22(b4) || a === b4;
}
...
var EXPECTED_COLOR = y2.green;
var RECEIVED_COLOR = y2.red;
var INVERTED_COLOR = y2.inverse;
var BOLD_WEIGHT = y2.bold;
var DIM_COLOR = y2.dim;
...
function getMatcherUtils() {
  return {
    ...
    printDiffOrStringify: printDiffOrStringify2,
    printWithType
  };
}
function printWithType(name, value, print) {
  const type3 = getType3(value);
...
function addCustomEqualityTesters(newTesters) {
  if (!Array.isArray(newTesters)) {
    throw new TypeError(`expect.customEqualityTesters: Must be set to an array of Testers. Was given "${getType3(newTesters)}"`);
  }
...
function equals(a, b4, customTesters, strictCheck) {
  customTesters = customTesters || [];
  return eq(a, b4, [], [], customTesters, strictCheck ? hasKey : hasDefinedKey);
}
var functionToString = Function.prototype.toString;
function isAsymmetric(obj) {
  return !!obj && typeof obj === "object" && "asymmetricMatch" in obj && isA("Function", obj.asymmetricMatch);
}
function asymmetricMatch(a, b4, customTesters) {
  const asymmetricA = isAsymmetric(a);
  const asymmetricB = isAsymmetric(b4);
  if (asymmetricA && asymmetricB) {
    return void 0;
  }
  if (asymmetricA) {
    return a.asymmetricMatch(b4, customTesters);
  }
  if (asymmetricB) {
    return b4.asymmetricMatch(a, customTesters);
  }
}
...
function eq(a, b4, aStack, bStack, customTesters, hasKey2) {
  let result = true;
  const asymmetricResult = asymmetricMatch(a, b4, customTesters);
  if (asymmetricResult !== void 0) {
    return asymmetricResult;
  }
  const testerContext = { equals };
  for (let i4 = 0; i4 < customTesters.length; i4++) {
    const customTesterResult = customTesters[i4].call(testerContext, a, b4, customTesters);
    if (customTesterResult !== void 0) {
      return customTesterResult;
    }
  }
  if (typeof URL === "function" && a instanceof URL && b4 instanceof URL) {
    return a.href === b4.href;
  }
  if (Object.is(a, b4)) {
    return true;
  }
  ...
  if (className !== Object.prototype.toString.call(b4)) {
    return false;
  }
  switch (className) {
    case "[object Boolean]":
    case "[object String]":
    case "[object Number]":
      if (typeof a !== typeof b4) {
        return false;
      } else if (typeof a !== "object" && typeof b4 !== "object") {
        return Object.is(a, b4);
      } else {
        return Object.is(a.valueOf(), b4.valueOf());
      }
    case "[object Date]": {
      const numA = +a;
      const numB = +b4;
    ...
    case "[object RegExp]":
      return a.source === b4.source && a.flags === b4.flags;
    ...
    case "[object Temporal.Duration]":
      return a.toString() === b4.toString();
  }
  if (typeof a !== "object" || typeof b4 !== "object") {
    return false;
  }
  if (isDomNode(a) && isDomNode(b4)) {
    return a.isEqualNode(b4);
  }
  let length = aStack.length;
  while (length--) {
    if (aStack[length] === a) {
      return bStack[length] === b4;
    } else if (bStack[length] === b4) {
      return false;
    }
  }
  aStack.push(a);
  bStack.push(b4);
  if (className === "[object Array]" && a.length !== b4.length) {
    return false;
  }
  if (isError(a) && isError(b4)) {
    try {
      return isErrorEqual(a, b4, aStack, bStack, customTesters, hasKey2);
    } finally {
      aStack.pop();
      bStack.pop();
    }
  ...
  if (keys(b4, hasKey2).length !== size) {
    return false;
  }
  while (size--) {
    key = aKeys[size];
    result = hasKey2(b4, key) && eq(a[key], b4[key], aStack, bStack, customTesters, hasKey2);
    if (!result) {
      return false;
    }
  }
...
function isErrorEqual(a, b4, aStack, bStack, customTesters, hasKey2) {
  let result = Object.prototype.toString.call(a) === Object.prototype.toString.call(b4) && a.name === b4.name && a.message === b4.message;
  if (typeof b4.cause !== "undefined") {
    result && (result = eq(a.cause, b4.cause, aStack, bStack, customTesters, hasKey2));
  }
  if (a instanceof AggregateError && b4 instanceof AggregateError) {
    result && (result = eq(a.errors, b4.errors, aStack, bStack, customTesters, hasKey2));
  }
  result && (result = eq({ ...a }, { ...b4 }, aStack, bStack, customTesters, hasKey2));
  return result;
}
...
function isImmutableUnorderedKeyed(maybeKeyed) {
  return !!(maybeKeyed && maybeKeyed[IS_KEYED_SENTINEL3] && !maybeKeyed[IS_ORDERED_SENTINEL3]);
}
function isImmutableUnorderedSet(maybeSet) {
  return !!(maybeSet && maybeSet[IS_SET_SENTINEL3] && !maybeSet[IS_ORDERED_SENTINEL3]);
}
function isObjectLiteral(source) {
  return source != null && typeof source === "object" && !Array.isArray(source);
}
function isImmutableList(source) {
  return Boolean(source && isObjectLiteral(source) && source[IS_LIST_SENTINEL3]);
}
function isImmutableOrderedKeyed(source) {
  return Boolean(source && isObjectLiteral(source) && source[IS_KEYED_SENTINEL3] && source[IS_ORDERED_SENTINEL3]);
}
function isImmutableOrderedSet(source) {
  return Boolean(source && isObjectLiteral(source) && source[IS_SET_SENTINEL3] && source[IS_ORDERED_SENTINEL3]);
}
...
function iterableEquality(a, b4, customTesters = [], aStack = [], bStack = []) {
  if (typeof a !== "object" || typeof b4 !== "object" || Array.isArray(a) || Array.isArray(b4) || !hasIterator(a) || !hasIterator(b4)) {
    return void 0;
  }
  if (a.constructor !== b4.constructor) {
    return false;
  }
  let length = aStack.length;
  while (length--) {
  ...
  function iterableEqualityWithStack(a2, b5) {
    return iterableEquality(a2, b5, [...customTesters], [...aStack], [...bStack]);
  }
  if (a.size !== void 0) {
    if (a.size !== b4.size) {
      return false;
      ...
      for (const aEntry of a) {
        if (!b4.has(aEntry[0]) || !equals(aEntry[1], b4.get(aEntry[0]), filteredCustomTesters)) {
          let has = false;
          for (const bEntry of b4) {
  ...
  const bIterator = b4[IteratorSymbol]();
  for (const aValue of a) {
  ...
  if (!isImmutableList(a) && !isImmutableOrderedKeyed(a) && !isImmutableOrderedSet(a) && !isImmutableRecord(a)) {
    ...
    const bEntries = Object.entries(b4);
...
function isObjectWithKeys(a) {
  return isObject2(a) && !isError(a) && !Array.isArray(a) && !(a instanceof Date) && !(a instanceof Set) && !(a instanceof Map);
}
...
function typeEquality(a, b4) {
  if (a == null || b4 == null || a.constructor === b4.constructor) {
    return void 0;
  }
  return false;
}
function arrayBufferEquality(a, b4) {
  let dataViewA = a;
  let dataViewB = b4;
  if (!(a instanceof DataView && b4 instanceof DataView)) {
    if (!(a instanceof ArrayBuffer) || !(b4 instanceof ArrayBuffer)) {
      return void 0;
    }
    try {
      dataViewA = new DataView(a);
      dataViewB = new DataView(b4);
    } catch {
      return void 0;
    }
  }
  ...
  for (let i4 = 0; i4 < dataViewA.byteLength; i4++) {
    if (dataViewA.getUint8(i4) !== dataViewB.getUint8(i4)) {
      return false;
    }
  }
  return true;
}
function sparseArrayEquality(a, b4, customTesters = []) {
  ...
  return equals(a, b4, filteredCustomTesters, true) && equals(aKeys, bKeys);
}
  ...
  const getObjectSubsetWithContext = (seenReferences = /* @__PURE__ */ new WeakMap()) => (object3, subset2) => {
    if (Array.isArray(object3)) {
      if (Array.isArray(subset2) && subset2.length === object3.length) {
        return subset2.map((sub, i4) => getObjectSubsetWithContext(seenReferences)(object3[i4], sub));
      }
    } else if (object3 instanceof Date) {
      return object3;
    } else if (isObject2(object3) && isObject2(subset2)) {
...
AsymmetricMatcher5.prototype[Symbol.for("chai/inspect")] = function(options) {
...
var StringContaining = class extends AsymmetricMatcher5 {
  constructor(sample, inverse = false) {
...
var Anything = class extends AsymmetricMatcher5 {
  asymmetricMatch(other) {
    return other != null;
  }
  toString() {
...
var ObjectContaining = class extends AsymmetricMatcher5 {
  constructor(sample, inverse = false) {
    super(sample, inverse);
  }
  getPrototype(obj) {
...
var ArrayContaining = class extends AsymmetricMatcher5 {
  constructor(sample, inverse = false) {
    super(sample, inverse);
  }
  asymmetricMatch(other, customTesters) {
...
var Any = class extends AsymmetricMatcher5 {
  constructor(sample) {
...
var StringMatching = class extends AsymmetricMatcher5 {
  constructor(sample, inverse = false) {
...
var CloseTo = class extends AsymmetricMatcher5 {
...
var SchemaMatching = class extends AsymmetricMatcher5 {
  constructor(sample, inverse = false) {
...
function handleTestError(test7, err) {
  ...
  (_a = test7.result).errors || (_a.errors = []);
  test7.result.errors.push(processError2(err));
}
...
(${stripped} matching ${stripped === 1 ? "property" : "properties"} omitted from actual)`;
  ...
  def("toContain", function(item) {
    ...
    if (typeof DOMTokenList !== "undefined" && actual instanceof DOMTokenList) {
      assertTypes2(item, "class name", ["string"]);
      const isNot = utils.flag(this, "negate");
  ...
  def("toContainEqual", function(expected) {
    const obj = utils.flag(this, "object");
    const index5 = Array.from(obj).findIndex((item) => {
      return equals(item, expected, customTesters);
    });
    this.assert(index5 !== -1, "expected #{this} to deep equally contain #{exp}", "expected #{this} to not deep equally contain #{exp}", expected);
  });
  ...
  def("toBeGreaterThan", function(expected) {
    const actual = this._obj;
    assertTypes2(actual, "actual", ["number", "bigint"]);
    assertTypes2(expected, "expected", ["number", "bigint"]);
  ...
  function equalsArgumentArray(a, b4) {
    return a.length === b4.length && a.every((aItem, i4) => equals(aItem, b4[i4], [...customTesters, iterableEquality]));
  }
  ...
  def("toHaveBeenNthCalledWith", function(times, ...args) {
    ...
    this.assert(nthCall && equalsArgumentArray(nthCall, args), `expected ${ordinal2(times)} "${spyName}" call to have been called with #{exp}${isCalled ? `` : `, but called only ${callCount} times`}`, `expected ${ordinal2(times)} "${spyName}" call to not have been called with #{exp}`, args, nthCall, isCalled);
  });
  ...
  [{
    ...
    condition: (spy, index5, value) => {
      const result = spy.mock.settledResults[index5 - 1];
    ...
    condition: (spy, index5, value) => {
      const result = spy.mock.results[index5 - 1];
    ...
    def(name, function(nthCall, value) {
      ...
      const ordinalCall = `${ordinal2(nthCall)} call`;
  ...
  utils.addProperty(chai.Assertion.prototype, "resolves", function __VITEST_RESOLVES__() {
    ...
    const proxy = new Proxy(this, { get: (target, key, receiver) => {
      ...
      return (...args) => {
        ...
        return recordAsyncExpect(test7, promise, createAssertionMessage(utils, this, !!args.length), error, utils.flag(this, "soft"));
      };
    } });
    return proxy;
  });
...
${spy.mock.calls.map((callArg, i4) => {
      let methodCall = y2.bold(`  ${ordinal2(i4 + 1)} ${spy.getMockName()} call:
...
Number of calls: ${y2.bold(spy.mock.calls.length)}
`);
  return msg;
}
...
${results.map((callReturn, i4) => {
      let methodCall = y2.bold(`  ${ordinal2(i4 + 1)} ${spy.getMockName()} call return:
...
function JestExtendPlugin(c, expect2, matchers) {
  return (_, utils) => {
    Object.entries(matchers).forEach(([expectAssertionName, expectAssertion]) => {
      ...
      class CustomMatcher extends AsymmetricMatcher5 {
...
// node_modules/vitest/node_modules/@vitest/utils/dist/chunk-pathe.M-eThtNZ.js
...
var resolve3 = function(...arguments_) {
  ...
  let resolvedAbsolute = false;
  for (let index5 = arguments_.length - 1; index5 >= -1 && !resolvedAbsolute; index5--) {
    const path = index5 >= 0 ? arguments_[index5] : cwd3();
    if (!path || path.length === 0) {
      continue;
    }
    resolvedPath = `${path}/${resolvedPath}`;
...
for (let i4 = 0; i4 < chars2.length; i4++) {
  const c = chars2.charCodeAt(i4);
  intToChar2[i4] = c;
  charToInt2[c] = i4;
}
var CHROME_IE_STACK_REGEXP2 = /^\s*at .*(?:\S:\d+|\(native\))/m;
var SAFARI_NATIVE_CODE_REGEXP2 = /^(?:eval@)?(?:\[native code\])?$/;
var NOW_LENGTH2 = Date.now().toString().length;
var REGEXP_VITEST2 = new RegExp(`vitest=\\d{${NOW_LENGTH2}}`);
function extractLocation2(urlLike) {
  ...
  if (url.includes("vitest=")) {
    url = url.replace(REGEXP_VITEST2, "").replace(/[?&]$/, "");
  }
  return [
    url,
    parts[2] || void 0,
...
function parseSingleV8Stack2(raw) {
  ...
  file = file.startsWith("node:") || file.startsWith("internal:") ? file : resolve3(file);
  if (method) {
    method = method.replace(/\(0\s?,\s?__vite_ssr_import_\d+__.(\w+)\)/g, "$1").replace(/__(vite_ssr_import|vi_import)_\d+__\./g, "").replace(/(Object\.)?__vite_ssr_export_default__\s?/g, "");
  }
  return {
  ...
  constructor(y4, m, d4, h, M, s, ms) {
    ...
    switch (arguments.length) {
      ...
      case 1:
        ...
        break;
      default:
        d4 = typeof d4 === "undefined" ? 1 : d4;
        h = h || 0;
        M = M || 0;
        s = s || 0;
        ms = ms || 0;
        date = new RealDate(y4, m, d4, h, M, s, ms);
        break;
    }
...
// node_modules/@vitest/snapshot/node_modules/@vitest/utils/dist/constants.js
var KNOWN_ASSET_TYPES3 = [
  "apng",
  "bmp",
  "png",
  "jpe?g",
...
var KNOWN_ASSET_RE3 = new RegExp(`\\.(${KNOWN_ASSET_TYPES3.join("|")})$`);

// node_modules/@vitest/snapshot/node_modules/@vitest/utils/dist/helpers.js
...
// node_modules/@vitest/snapshot/node_modules/@vitest/utils/dist/chunk-pathe.M-eThtNZ.js
var _DRIVE_LETTER_START_RE4 = /^[A-Za-z]:\//;
function normalizeWindowsPath4(input = "") {
  if (!input) {
    return input;
  }
  return input.replace(/\\/g, "/").replace(_DRIVE_LETTER_START_RE4, (r) => r.toUpperCase());
}
var _IS_ABSOLUTE_RE4 = /^[/\\](?![/\\])|^[/\\]{2}(?!\.)|^[A-Za-z]:[/\\]/;
function cwd4() {
  if (typeof process !== "undefined" && typeof process.cwd === "function") {
    return process.cwd().replace(/\\/g, "/");
  }
...
var resolve4 = function(...arguments_) {
  arguments_ = arguments_.map((argument) => normalizeWindowsPath4(argument));
  let resolvedPath = "";
  let resolvedAbsolute = false;
  for (let index5 = arguments_.length - 1; index5 >= -1 && !resolvedAbsolute; index5--) {
    const path = index5 >= 0 ? arguments_[index5] : cwd4();
    if (!path || path.length === 0) {
      continue;
    }
    resolvedPath = `${path}/${resolvedPath}`;
    resolvedAbsolute = isAbsolute4(path);
  }
  resolvedPath = normalizeString4(resolvedPath, !resolvedAbsolute);
  if (resolvedAbsolute && !isAbsolute4(resolvedPath)) {
    return `/${resolvedPath}`;
  }
  return resolvedPath.length > 0 ? resolvedPath : ".";
};
function normalizeString4(path, allowAboveRoot) {
  let res = "";
  let lastSegmentLength = 0;
  let lastSlash = -1;
  let dots = 0;
...
var isAbsolute4 = function(p) {
  return _IS_ABSOLUTE_RE4.test(p);
};

// node_modules/@vitest/snapshot/node_modules/@vitest/utils/dist/source-map.js
...
for (let i4 = 0; i4 < chars3.length; i4++) {
  const c = chars3.charCodeAt(i4);
  intToChar3[i4] = c;
  charToInt3[c] = i4;
}
...
function upperBound(haystack, needle, index5) {
  for (let i4 = index5 + 1; i4 < haystack.length; index5 = i4++) {
    if (haystack[i4][COLUMN] !== needle) break;
  }
  return index5;
}
function lowerBound(haystack, needle, index5) {
  for (let i4 = index5 - 1; i4 >= 0; index5 = i4--) {
    if (haystack[i4][COLUMN] !== needle) break;
  }
  return index5;
}
...
function originalPositionFor(map2, needle) {
  ...
  const index5 = traceSegmentInternal(
    segments,
  ...
  if (index5 === -1) return OMapping(null, null, null, null);
  const segment = segments[index5];
...
function traceSegmentInternal(segments, memo2, line, column, bias) {
  let index5 = memoizedBinarySearch(segments, column, memo2, line);
  if (found) {
    index5 = (bias === LEAST_UPPER_BOUND ? upperBound : lowerBound)(segments, column, index5);
  } else if (bias === LEAST_UPPER_BOUND) index5++;
  if (index5 === -1 || index5 === segments.length) return -1;
  return index5;
}
...
var NOW_LENGTH3 = Date.now().toString().length;
var REGEXP_VITEST3 = new RegExp(`vitest=\\d{${NOW_LENGTH3}}`);
function extractLocation3(urlLike) {
  ...
  if (url.includes("vitest=")) {
    url = url.replace(REGEXP_VITEST3, "").replace(/[?&]$/, "");
  }
  return [
    url,
    parts[2] || void 0,
...
function parseSingleFFOrSafariStack3(raw) {
  ...
  const [url, lineNumber, columnNumber] = extractLocation3(locationPart);
  if (!url || !lineNumber || !columnNumber) {
    return null;
  }
  return {
...
function parseStacktrace(stack, options = {}) {
  ...
  let stacks = !CHROME_IE_STACK_REGEXP3.test(stack) ? parseFFOrSafariStackTrace(stack) : parseV8Stacktrace(stack);
...
function parseFFOrSafariStackTrace(stack) {
  return stack.split("\n").map((line) => parseSingleFFOrSafariStack3(line)).filter(notNullish3);
}
function parseV8Stacktrace(stack) {
  return stack.split("\n").map((line) => parseSingleV8Stack3(line)).filter(notNullish3);
}
...
var DecodedMap = class {
  constructor(map2, from) {
    ...
    this.url = from;
    this.resolvedSources = (sources || []).map((s) => resolve4(from, "..", s || ""));
  }
};
...
// node_modules/@vitest/snapshot/node_modules/@vitest/utils/dist/offset.js
var lineSplitRE = /\r?\n/;
function positionToOffset(source, lineNumber, columnNumber) {
  ...
  for (let i4 = 0; i4 < lineNumber - 1; i4++) {
    start += lines[i4].length + nl;
  }
...
// node_modules/@vitest/snapshot/node_modules/tinyrainbow/dist/index.js
var b3 = {
  reset: [0, 0],
  bold: [1, 22, "\x1B[22m\x1B[1m"],
  dim: [2, 22, "\x1B[22m\x1B[2m"],
...
function _mergeNamespaces3(n, m) {
  m.forEach(function(e) {
    e && typeof e !== "string" && !Array.isArray(e) && Object.keys(e).forEach(function(k) {
      if (k !== "default" && !(k in n)) {
        var d4 = Object.getOwnPropertyDescriptor(e, k);
...
function printIteratorEntries3(iterator, config2, indentation, depth, refs, printer4, separator = ": ") {
  let result = "";
  let width = 0;
  let current = iterator.next();
  if (!current.done) {
...
function printIteratorValues3(iterator, config2, indentation, depth, refs, printer4) {
  let result = "";
  let width = 0;
  let current = iterator.next();
  if (!current.done) {
...
function printListItems3(list, config2, indentation, depth, refs, printer4) {
  let result = "";
  list = list instanceof ArrayBuffer ? new DataView(list) : list;
  const isDataView = (l) => l instanceof DataView;
  const length = isDataView(list) ? list.byteLength : list.length;
...
function printObjectProperties3(val, config2, indentation, depth, refs, printer4) {
  let result = "";
...
var SPACE$23 = " ";
var serialize$53 = (val, config2, indentation, depth, refs, printer4) => {
  const stringedValue = val.toString();
  if (stringedValue === "ArrayContaining" || stringedValue === "ArrayNotContaining") {
    if (++depth > config2.maxDepth) {
      return `[${stringedValue}]`;
    }
    return `${stringedValue + SPACE$23}[${printListItems3(val.sample, config2, indentation, depth, refs, printer4)}]`;
  }
  if (stringedValue === "ObjectContaining" || stringedValue === "ObjectNotContaining") {
    if (++depth > config2.maxDepth) {
      return `[${stringedValue}]`;
    }
    return `${stringedValue + SPACE$23}{${printObjectProperties3(val.sample, config2, indentation, depth, refs, printer4)}}`;
  }
  if (stringedValue === "StringMatching" || stringedValue === "StringNotMatching") {
    return stringedValue + SPACE$23 + printer4(val.sample, config2, indentation, depth, refs);
  }
  if (stringedValue === "StringContaining" || stringedValue === "StringNotContaining") {
    return stringedValue + SPACE$23 + printer4(val.sample, config2, indentation, depth, refs);
  }
...
var serialize$43 = (collection, config2, indentation, depth, refs, printer4) => {
  const name = collection.constructor.name;
  if (++depth > config2.maxDepth) {
    return `[${name}]`;
  }
  return (config2.min ? "" : name + SPACE$13) + (OBJECT_NAMES3.has(name) ? `{${printObjectProperties3(isNamedNodeMap3(collection) ? [...collection].reduce((props, attribute) => {
    props[attribute.name] = attribute.value;
    return props;
  }, {}) : { ...collection }, config2, indentation, depth, refs, printer4)}}` : `[${printListItems3([...collection], config2, indentation, depth, refs, printer4)}]`);
};
var plugin$43 = {
...
function printProps3(keys2, props, config2, indentation, depth, refs, printer4) {
  const indentationNext = indentation + config2.indent;
  const colors = config2.colors;
  return keys2.map((key) => {
    const value = props[key];
...
function printChildren3(children, config2, indentation, depth, refs, printer4) {
  return children.map((child) => config2.spacingOuter + indentation + (typeof child === "string" ? printText3(child, config2) : printer4(child, config2, indentation, depth, refs))).join("");
}
function printShadowRoot3(children, config2, indentation, depth, refs, printer4) {
  if (config2.printShadowRoot === false) {
    return "";
  }
  return [`${config2.spacingOuter + indentation}#shadow-root`, printChildren3(children, config2, indentation + config2.indent, depth, refs, printer4)].join("");
}
...
var test$33 = (val) => {
  var _a;
  return ((_a = val == null ? void 0 : val.constructor) == null ? void 0 : _a.name) && testNode3(val);
};
...
function filterChildren3(children, filterNode) {
  let filtered = children.filter((node) => {
...
function serializeDOM3(node, config2, indentation, depth, refs, printer4, filterNode) {
  if (nodeIsText3(node)) {
  ...
  const children = Array.prototype.slice.call(node.childNodes || node.children);
  const shadowChildren = nodeIsFragment3(node) || !node.shadowRoot ? [] : Array.prototype.slice.call(node.shadowRoot.children);
  const resolvedChildren = filterNode ? filterChildren3(children, filterNode) : children;
  const resolvedShadowChildren = filterNode ? filterChildren3(shadowChildren, filterNode) : shadowChildren;
  return printElement3(type3, printProps3(nodeIsFragment3(node) ? [] : Array.from(node.attributes, (attr) => attr.name).sort(), nodeIsFragment3(node) ? {} : [...node.attributes].reduce((props, attribute) => {
    props[attribute.name] = attribute.value;
    return props;
  }, {}), config2, indentation + config2.indent, depth, refs, printer4), (resolvedShadowChildren.length > 0 ? printShadowRoot3(resolvedShadowChildren, config2, indentation + config2.indent, depth, refs, printer4) : "") + printChildren3(resolvedChildren, config2, indentation + config2.indent, depth, refs, printer4), config2, indentation);
}
var serialize$33 = (node, config2, indentation, depth, refs, printer4) => serializeDOM3(node, config2, indentation, depth, refs, printer4);
var plugin$33 = {
...
var SPACE3 = " ";
var LAZY3 = "…";
function printImmutableEntries3(val, config2, indentation, depth, refs, printer4, type3) {
  return ++depth > config2.maxDepth ? printAsLeaf3(getImmutableName3(type3)) : `${getImmutableName3(type3) + SPACE3}{${printIteratorEntries3(val.entries(), config2, indentation, depth, refs, printer4)}}`;
}
...
function printImmutableRecord3(val, config2, indentation, depth, refs, printer4) {
  const name = getImmutableName3(val._name || "Record");
  return ++depth > config2.maxDepth ? printAsLeaf3(name) : `${name + SPACE3}{${printIteratorEntries3(getRecordEntries3(val), config2, indentation, depth, refs, printer4)}}`;
}
function printImmutableSeq3(val, config2, indentation, depth, refs, printer4) {
  const name = getImmutableName3("Seq");
  if (++depth > config2.maxDepth) {
    return printAsLeaf3(name);
  }
  if (val[IS_KEYED_SENTINEL4]) {
    return `${name + SPACE3}{${val._iter || val._object ? printIteratorEntries3(val.entries(), config2, indentation, depth, refs, printer4) : LAZY3}}`;
  }
  return `${name + SPACE3}[${val._iter || val._array || val._collection || val._iterable ? printIteratorValues3(val.values(), config2, indentation, depth, refs, printer4) : LAZY3}]`;
}
function printImmutableValues3(val, config2, indentation, depth, refs, printer4, type3) {
  return ++depth > config2.maxDepth ? printAsLeaf3(getImmutableName3(type3)) : `${getImmutableName3(type3) + SPACE3}[${printIteratorValues3(val.values(), config2, indentation, depth, refs, printer4)}]`;
}
var serialize$23 = (val, config2, indentation, depth, refs, printer4) => {
  if (val[IS_MAP_SENTINEL3]) {
    return printImmutableEntries3(val, config2, indentation, depth, refs, printer4, val[IS_ORDERED_SENTINEL4] ? "OrderedMap" : "Map");
  }
  if (val[IS_LIST_SENTINEL4]) {
    return printImmutableValues3(val, config2, indentation, depth, refs, printer4, "List");
  }
  if (val[IS_SET_SENTINEL4]) {
    return printImmutableValues3(val, config2, indentation, depth, refs, printer4, val[IS_ORDERED_SENTINEL4] ? "OrderedSet" : "Set");
  }
  if (val[IS_STACK_SENTINEL3]) {
    return printImmutableValues3(val, config2, indentation, depth, refs, printer4, "Stack");
  }
  if (val[IS_SEQ_SENTINEL3]) {
    return printImmutableSeq3(val, config2, indentation, depth, refs, printer4);
  }
  return printImmutableRecord3(val, config2, indentation, depth, refs, printer4);
};
...
var reactIs_production3 = {};
var hasRequiredReactIs_production3;
function requireReactIs_production3() {
  if (hasRequiredReactIs_production3) return reactIs_production3;
  hasRequiredReactIs_production3 = 1;
  var REACT_ELEMENT_TYPE = Symbol.for("react.transitional.element"), REACT_PORTAL_TYPE = Symbol.for("react.portal"), REACT_FRAGMENT_TYPE = Symbol.for("react.fragment"), REACT_STRICT_MODE_TYPE = Symbol.for("react.strict_mode"), REACT_PROFILER_TYPE = Symbol.for("react.profiler"), REACT_CONSUMER_TYPE = Symbol.for("react.consumer"), REACT_CONTEXT_TYPE = Symbol.for("react.context"), REACT_FORWARD_REF_TYPE = Symbol.for("react.forward_ref"), REACT_SUSPENSE_TYPE = Symbol.for("react.suspense"), REACT_SUSPENSE_LIST_TYPE = Symbol.for("react.suspense_list"), REACT_MEMO_TYPE = Symbol.for("react.memo"), REACT_LAZY_TYPE = Symbol.for("react.lazy"), REACT_VIEW_TRANSITION_TYPE = Symbol.for("react.view_transition"), REACT_CLIENT_REFERENCE = Symbol.for("react.client.reference");
  function typeOf2(object2) {
    if ("object" === typeof object2 && null !== object2) {
      var $$typeof = object2.$$typeof;
  ...
  reactIs_production3.ContextConsumer = REACT_CONSUMER_TYPE;
  reactIs_production3.ContextProvider = REACT_CONTEXT_TYPE;
  reactIs_production3.Element = REACT_ELEMENT_TYPE;
  reactIs_production3.ForwardRef = REACT_FORWARD_REF_TYPE;
  reactIs_production3.Fragment = REACT_FRAGMENT_TYPE;
  reactIs_production3.Lazy = REACT_LAZY_TYPE;
  reactIs_production3.Memo = REACT_MEMO_TYPE;
  reactIs_production3.Portal = REACT_PORTAL_TYPE;
  reactIs_production3.Profiler = REACT_PROFILER_TYPE;
  reactIs_production3.StrictMode = REACT_STRICT_MODE_TYPE;
  reactIs_production3.Suspense = REACT_SUSPENSE_TYPE;
  reactIs_production3.SuspenseList = REACT_SUSPENSE_LIST_TYPE;
  reactIs_production3.isContextConsumer = function(object2) {
    return typeOf2(object2) === REACT_CONSUMER_TYPE;
  };
  reactIs_production3.isContextProvider = function(object2) {
    return typeOf2(object2) === REACT_CONTEXT_TYPE;
  };
  reactIs_production3.isElement = function(object2) {
    return "object" === typeof object2 && null !== object2 && object2.$$typeof === REACT_ELEMENT_TYPE;
  };
  reactIs_production3.isForwardRef = function(object2) {
    return typeOf2(object2) === REACT_FORWARD_REF_TYPE;
  };
  reactIs_production3.isFragment = function(object2) {
    return typeOf2(object2) === REACT_FRAGMENT_TYPE;
  };
  reactIs_production3.isLazy = function(object2) {
    return typeOf2(object2) === REACT_LAZY_TYPE;
  };
  reactIs_production3.isMemo = function(object2) {
    return typeOf2(object2) === REACT_MEMO_TYPE;
  };
  reactIs_production3.isPortal = function(object2) {
    return typeOf2(object2) === REACT_PORTAL_TYPE;
  };
  reactIs_production3.isProfiler = function(object2) {
    return typeOf2(object2) === REACT_PROFILER_TYPE;
  };
  reactIs_production3.isStrictMode = function(object2) {
    return typeOf2(object2) === REACT_STRICT_MODE_TYPE;
  };
  reactIs_production3.isSuspense = function(object2) {
    return typeOf2(object2) === REACT_SUSPENSE_TYPE;
  };
  reactIs_production3.isSuspenseList = function(object2) {
    return typeOf2(object2) === REACT_SUSPENSE_LIST_TYPE;
  };
  reactIs_production3.isValidElementType = function(type3) {
    return "string" === typeof type3 || "function" === typeof type3 || type3 === REACT_FRAGMENT_TYPE || type3 === REACT_PROFILER_TYPE || type3 === REACT_STRICT_MODE_TYPE || type3 === REACT_SUSPENSE_TYPE || type3 === REACT_SUSPENSE_LIST_TYPE || "object" === typeof type3 && null !== type3 && (type3.$$typeof === REACT_LAZY_TYPE || type3.$$typeof === REACT_MEMO_TYPE || type3.$$typeof === REACT_CONTEXT_TYPE || type3.$$typeof === REACT_CONSUMER_TYPE || type3.$$typeof === REACT_FORWARD_REF_TYPE || type3.$$typeof === REACT_CLIENT_REFERENCE || void 0 !== type3.getModuleId) ? true : false;
  };
  reactIs_production3.typeOf = typeOf2;
  return reactIs_production3;
}
...
function requireReactIs$13() {
  ...
  {
    reactIs$13.exports = requireReactIs_production3();
  }
  return reactIs$13.exports;
}
var reactIsExports$13 = requireReactIs$13();
var index$13 = getDefaultExportFromCjs5(reactIsExports$13);
...
var hasRequiredReactIs_production_min3;
function requireReactIs_production_min3() {
  if (hasRequiredReactIs_production_min3) return reactIs_production_min3;
  hasRequiredReactIs_production_min3 = 1;
  var b4 = Symbol.for("react.element"), c = Symbol.for("react.portal"), d4 = Symbol.for("react.fragment"), e = Symbol.for("react.strict_mode"), f = Symbol.for("react.profiler"), g = Symbol.for("react.provider"), h = Symbol.for("react.context"), k = Symbol.for("react.server_context"), l = Symbol.for("react.forward_ref"), m = Symbol.for("react.suspense"), n = Symbol.for("react.suspense_list"), p = Symbol.for("react.memo"), q = Symbol.for("react.lazy"), t = Symbol.for("react.offscreen"), u;
  u = Symbol.for("react.module.reference");
  function v(a) {
    if ("object" === typeof a && null !== a) {
  ...
  reactIs_production_min3.ContextConsumer = h;
  reactIs_production_min3.ContextProvider = g;
  reactIs_production_min3.Element = b4;
  reactIs_production_min3.ForwardRef = l;
  reactIs_production_min3.Fragment = d4;
  reactIs_production_min3.Lazy = q;
  reactIs_production_min3.Memo = p;
  reactIs_production_min3.Portal = c;
  reactIs_production_min3.Profiler = f;
  reactIs_production_min3.StrictMode = e;
  reactIs_production_min3.Suspense = m;
  reactIs_production_min3.SuspenseList = n;
  reactIs_production_min3.isAsyncMode = function() {
    return false;
  };
  reactIs_production_min3.isConcurrentMode = function() {
    return false;
  };
  reactIs_production_min3.isContextConsumer = function(a) {
    return v(a) === h;
  };
  reactIs_production_min3.isContextProvider = function(a) {
    return v(a) === g;
  };
  reactIs_production_min3.isElement = function(a) {
    return "object" === typeof a && null !== a && a.$$typeof === b4;
  };
  reactIs_production_min3.isForwardRef = function(a) {
    return v(a) === l;
  };
  reactIs_production_min3.isFragment = function(a) {
    return v(a) === d4;
  };
  reactIs_production_min3.isLazy = function(a) {
    return v(a) === q;
  };
  reactIs_production_min3.isMemo = function(a) {
    return v(a) === p;
  };
  reactIs_production_min3.isPortal = function(a) {
    return v(a) === c;
  };
  reactIs_production_min3.isProfiler = function(a) {
    return v(a) === f;
  };
  reactIs_production_min3.isStrictMode = function(a) {
    return v(a) === e;
  };
  reactIs_production_min3.isSuspense = function(a) {
    return v(a) === m;
  };
  reactIs_production_min3.isSuspenseList = function(a) {
    return v(a) === n;
  };
  reactIs_production_min3.isValidElementType = function(a) {
    return "string" === typeof a || "function" === typeof a || a === d4 || a === f || a === e || a === m || a === n || a === t || "object" === typeof a && null !== a && (a.$$typeof === q || a.$$typeof === p || a.$$typeof === g || a.$$typeof === h || a.$$typeof === l || a.$$typeof === u || void 0 !== a.getModuleId) ? true : false;
  };
  reactIs_production_min3.typeOf = v;
  return reactIs_production_min3;
}
...
function requireReactIs3() {
  ...
  {
    reactIs3.exports = requireReactIs_production_min3();
  }
  return reactIs3.exports;
}
var reactIsExports3 = requireReactIs3();
var index3 = getDefaultExportFromCjs5(reactIsExports3);
...
var ReactIs3 = Object.fromEntries(reactIsMethods3.map((m) => [m, (v) => ReactIs183[m](v) || ReactIs193[m](v)]));
...
var serialize$13 = (element, config2, indentation, depth, refs, printer4) => ++depth > config2.maxDepth ? printElementAsLeaf3(getType6(element), config2) : printElement3(getType6(element), printProps3(getPropKeys$13(element), element.props, config2, indentation + config2.indent, depth, refs, printer4), printChildren3(getChildren3(element.props.children), config2, indentation + config2.indent, depth, refs, printer4), config2, indentation);
...
var serialize3 = (object2, config2, indentation, depth, refs, printer4) => ++depth > config2.maxDepth ? printElementAsLeaf3(object2.type, config2) : printElement3(object2.type, object2.props ? printProps3(getPropKeys3(object2), object2.props, config2, indentation + config2.indent, depth, refs, printer4) : "", object2.children ? printChildren3(object2.children, config2, indentation + config2.indent, depth, refs, printer4) : "", config2, indentation);
...
function isWindow3(val) {
  return typeof window !== "undefined" && val === window;
}
var SYMBOL_REGEXP3 = /^Symbol\((.*)\)(.*)$/;
var NEWLINE_REGEXP3 = /\n/g;
var PrettyFormatPluginError3 = class extends Error {
  constructor(message, stack) {
    super(message);
    this.stack = stack;
    this.name = this.constructor.name;
...
function isToStringedArrayType3(toStringed) {
  return toStringed === "[object Array]" || toStringed === "[object ArrayBuffer]" || toStringed === "[object DataView]" || toStringed === "[object Float32Array]" || toStringed === "[object Float64Array]" || toStringed === "[object Int8Array]" || toStringed === "[object Int16Array]" || toStringed === "[object Int32Array]" || toStringed === "[object Uint8Array]" || toStringed === "[object Uint8ClampedArray]" || toStringed === "[object Uint16Array]" || toStringed === "[object Uint32Array]";
}
function printNumber3(val) {
  return Object.is(val, -0) ? "-0" : String(val);
}
function printBigInt3(val) {
  return String(`${val}n`);
}
function printFunction3(val, printFunctionName2) {
  if (!printFunctionName2) {
    return "[Function]";
  }
  return `[Function ${val.name || "anonymous"}]`;
}
function printSymbol3(val) {
  return String(val).replace(SYMBOL_REGEXP3, "Symbol($1)");
}
function printError3(val) {
  return `[${errorToString3.call(val)}]`;
}
function printBasicValue3(val, printFunctionName2, escapeRegex2, escapeString) {
  if (val === true || val === false) {
    return `${val}`;
  }
  if (val === void 0) {
  ...
  const typeOf2 = typeof val;
  if (typeOf2 === "number") {
    return printNumber3(val);
  }
  if (typeOf2 === "bigint") {
    return printBigInt3(val);
  }
  if (typeOf2 === "string") {
    if (escapeString) {
      return `"${val.replaceAll(/"|\\/g, "\\$&")}"`;
  ...
  if (typeOf2 === "function") {
    return printFunction3(val, printFunctionName2);
  }
  if (typeOf2 === "symbol") {
    return printSymbol3(val);
  }
...
function printComplexValue3(val, config2, indentation, depth, refs, hasCalledToJSON) {
  if (refs.includes(val)) {
    return "[Circular]";
  }
  refs = [...refs];
  ...
  const min = config2.min;
  if (config2.callToJSON && !hitMaxDepth && val.toJSON && typeof val.toJSON === "function" && !hasCalledToJSON) {
    return printer3(val.toJSON(), config2, indentation, depth, refs, true);
  }
  const toStringed = toString5.call(val);
  if (toStringed === "[object Arguments]") {
    return hitMaxDepth ? "[Arguments]" : `${min ? "" : "Arguments "}[${printListItems3(val, config2, indentation, depth, refs, printer3)}]`;
  }
  if (isToStringedArrayType3(toStringed)) {
    return hitMaxDepth ? `[${val.constructor.name}]` : `${min ? "" : !config2.printBasicPrototype && val.constructor.name === "Array" ? "" : `${val.constructor.name} `}[${printListItems3(val, config2, indentation, depth, refs, printer3)}]`;
  }
  if (toStringed === "[object Map]") {
    return hitMaxDepth ? "[Map]" : `Map {${printIteratorEntries3(val.entries(), config2, indentation, depth, refs, printer3, " => ")}}`;
  }
  if (toStringed === "[object Set]") {
    return hitMaxDepth ? "[Set]" : `Set {${printIteratorValues3(val.values(), config2, indentation, depth, refs, printer3)}}`;
  }
  return hitMaxDepth || isWindow3(val) ? `[${getConstructorName4(val)}]` : `${min ? "" : !config2.printBasicPrototype && getConstructorName4(val) === "Object" ? "" : `${getConstructorName4(val)} `}{${printObjectProperties3(val, config2, indentation, depth, refs, printer3)}}`;
}
var ErrorPlugin3 = {
  test: (val) => val && val instanceof Error,
  serialize(val, config2, indentation, depth, refs, printer4) {
    ...
    return hitMaxDepth ? `[${name}]` : `${name} {${printIteratorEntries3(Object.entries(entries).values(), config2, indentation, depth, refs, printer4)}}`;
  }
};
function isNewPlugin3(plugin5) {
  return plugin5.serialize != null;
}
function printPlugin3(plugin5, val, config2, indentation, depth, refs) {
  let printed;
  try {
    printed = isNewPlugin3(plugin5) ? plugin5.serialize(val, config2, indentation, depth, refs, printer3) : plugin5.print(val, (valChild) => printer3(valChild, config2, indentation, depth, refs), (str) => {
      const indentationNext = indentation + config2.indent;
      return indentationNext + str.replaceAll(NEWLINE_REGEXP3, `
${indentationNext}`);
    }, {
      edgeSpacing: config2.spacingOuter,
      min: config2.min,
    ...
    throw new PrettyFormatPluginError3(error.message, error.stack);
  }
  if (typeof printed !== "string") {
    throw new TypeError(`pretty-format: Plugin must return type "string" but instead returned "${typeof printed}".`);
  }
...
function findPlugin3(plugins4, val) {
  for (const plugin5 of plugins4) {
    try {
      if (plugin5.test(val)) {
        return plugin5;
...
function printer3(val, config2, indentation, depth, refs, hasCalledToJSON) {
  var _a;
  let result;
  const plugin5 = findPlugin3(config2.plugins, val);
  if (plugin5 !== null) {
    result = printPlugin3(plugin5, val, config2, indentation, depth, refs);
  } else {
    const basicResult = printBasicValue3(val, config2.printFunctionName, config2.escapeRegex, config2.escapeString);
    if (basicResult !== null) {
      result = basicResult;
    } else {
      result = printComplexValue3(val, config2, indentation, depth, refs, hasCalledToJSON);
    }
  }
  (_a = config2._outputLengthPerDepth)[depth] ?? (_a[depth] = 0);
  config2._outputLengthPerDepth[depth] += result.length;
...
function validateOptions3(options) {
  for (const key of Object.keys(options)) {
    if (!Object.hasOwn(DEFAULT_OPTIONS3, key)) {
      throw new Error(`pretty-format: Unknown option "${key}".`);
    }
  }
  if (options.min && options.indent !== void 0 && options.indent !== 0) {
...
function getPrintFunctionName3(options) {
  return (options == null ? void 0 : options.printFunctionName) ?? DEFAULT_OPTIONS3.printFunctionName;
}
function getEscapeRegex3(options) {
  return (options == null ? void 0 : options.escapeRegex) ?? DEFAULT_OPTIONS3.escapeRegex;
}
function getEscapeString3(options) {
  return (options == null ? void 0 : options.escapeString) ?? DEFAULT_OPTIONS3.escapeString;
}
function getConfig3(options) {
  return {
    callToJSON: (options == null ? void 0 : options.callToJSON) ?? DEFAULT_OPTIONS3.callToJSON,
    colors: (options == null ? void 0 : options.highlight) ? getColorsHighlight3() : getColorsEmpty3(),
    compareKeys: typeof (options == null ? void 0 : options.compareKeys) === "function" || (options == null ? void 0 : options.compareKeys) === null ? options.compareKeys : DEFAULT_OPTIONS3.compareKeys,
    escapeRegex: getEscapeRegex3(options),
    escapeString: getEscapeString3(options),
    indent: (options == null ? void 0 : options.min) ? "" : createIndent3((options == null ? void 0 : options.indent) ?? DEFAULT_OPTIONS3.indent),
    maxDepth: (options == null ? void 0 : options.maxDepth) ?? DEFAULT_OPTIONS3.maxDepth,
    maxWidth: (options == null ? void 0 : options.maxWidth) ?? DEFAULT_OPTIONS3.maxWidth,
    min: (options == null ? void 0 : options.min) ?? DEFAULT_OPTIONS3.min,
    plugins: (options == null ? void 0 : options.plugins) ?? DEFAULT_OPTIONS3.plugins,
    printBasicPrototype: (options == null ? void 0 : options.printBasicPrototype) ?? true,
    printFunctionName: getPrintFunctionName3(options),
    printShadowRoot: (options == null ? void 0 : options.printShadowRoot) ?? true,
    spacingInner: (options == null ? void 0 : options.min) ? " " : "\n",
    spacingOuter: (options == null ? void 0 : options.min) ? "" : "\n",
    maxOutputLength: (options == null ? void 0 : options.maxOutputLength) ?? DEFAULT_OPTIONS3.maxOutputLength,
    _outputLengthPerDepth: []
  };
}
function createIndent3(indent) {
  return Array.from({ length: indent + 1 }).join(" ");
}
function format5(val, options) {
  if (options) {
    ...
    if (options.plugins) {
      const plugin5 = findPlugin3(options.plugins, val);
      if (plugin5 !== null) {
        return printPlugin3(plugin5, val, getConfig3(options), "", 0, []);
      }
    }
  }
  const basicResult = printBasicValue3(val, getPrintFunctionName3(options), getEscapeRegex3(options), getEscapeString3(options));
  if (basicResult !== null) {
    return basicResult;
  }
  return printComplexValue3(val, getConfig3(options), "", 0, []);
}
var plugins3 = {
...
function requireNaturalCompare() {
  ...
  var naturalCompare2 = function(a, b4) {
    var i4, codeA, codeB = 1, posA = 0, posB = 0, alphabet = String.alphabet;
    function getCode(str, pos, code) {
      if (code) {
        for (i4 = pos; code = getCode(str, i4), code < 76 && code > 65; ) ++i4;
        return +str.slice(pos - 1, i4);
      }
    ...
    if ((a += "") != (b4 += "")) for (; codeB; ) {
      ...
      codeB = getCode(b4, posB++);
      if (codeA < 76 && codeB < 76 && codeA > 66 && codeB > 66) {
        codeA = getCode(a, posA, posA);
        codeB = getCode(b4, posB, posA = i4);
        posB = i4;
      }
...
var naturalCompare = getDefaultExportFromCjs6(naturalCompareExports);
var serialize$14 = (val, config2, indentation, depth, refs, printer4) => {
  const name = val.getMockName();
  const nameString = name === "vi.fn()" ? "" : ` ${name}`;
  let callsString = "";
  if (val.mock.calls.length !== 0) {
    const indentationNext = indentation + config2.indent;
    callsString = ` {${config2.spacingOuter}${indentationNext}"calls": ${printer4(val.mock.calls, config2, indentationNext, depth, refs)}${config2.min ? ", " : ","}${config2.spacingOuter}${indentationNext}"results": ${printer4(val.mock.results, config2, indentationNext, depth, refs)}${config2.min ? "" : ","}${config2.spacingOuter}${indentation}}`;
  }
...
function addSerializer(plugin5) {
  PLUGINS5 = [plugin5].concat(PLUGINS5);
}
...
function serialize4(val, indent = 2, formatOverrides = {}) {
  return normalizeNewlines(format5(val, {
    escapeRegex,
    indent,
...
function deepMergeArray(target = [], source = []) {
  ...
  source.forEach((sourceElement, index5) => {
    const targetElement = mergedOutput[index5];
    if (Array.isArray(target[index5])) {
      mergedOutput[index5] = deepMergeArray(target[index5], sourceElement);
    } else if (isObject3(targetElement)) {
      mergedOutput[index5] = deepMergeSnapshot(target[index5], sourceElement);
    } else {
      mergedOutput[index5] = sourceElement;
    }
  });
  return mergedOutput;
}
...
async function saveInlineSnapshots(environment, snapshots) {
  ...
  const files = new Set(snapshots.map((i4) => i4.file));
  await Promise.all(Array.from(files).map(async (file) => {
    const snaps = snapshots.filter((i4) => i4.file === file);
    ...
    for (const snap of snaps) {
      const index5 = positionToOffset(code, snap.line, snap.column);
      replaceInlineSnap(code, s, index5, snap.snapshot, snap.assertionName);
    }
...
function replaceObjectSnap(code, s, index5, newSnap, assertionName) {
  let _code = code.slice(index5);
  ...
  callEnd += index5 + startMatch.index;
  const shapeStart = index5 + startMatch.index + startMatch[0].length;
  const shapeEnd = getObjectShapeEndIndex(code, shapeStart);
  const snap = `, ${prepareSnapString(newSnap, code, index5)}`;
  if (shapeEnd === callEnd) {
...
function getObjectShapeEndIndex(code, index5) {
  let startBraces = 1;
  let endBraces = 0;
  while (startBraces !== endBraces && index5 < code.length) {
    const s = code[index5++];
    if (s === "{") {
      startBraces++;
    } else if (s === "}") {
...
function prepareSnapString(snap, source, index5) {
  const lineNumber = offsetToLineNumber(source, index5);
  ...
  return `${quote}
${lines.map((i4) => i4 ? indentNext + i4 : "").join("\n").replace(/`/g, "\\`").replace(/\$\{/g, "\\${")}
${indent}${quote}`;
}
var defaultMethodNames = ["toMatchInlineSnapshot", "toThrowErrorMatchingInlineSnapshot"];
function getCodeStartingAtIndex(code, index5, methodNames) {
  for (const name of methodNames) {
    const adjusted = index5 - name.length;
    if (adjusted >= 0 && code.slice(adjusted, index5) === name) {
      return {
...
function replaceInlineSnap(code, s, currentIndex, newSnap, assertionName) {
  ...
  const { code: codeStartingAtIndex, index: index5 } = getCodeStartingAtIndex(code, currentIndex, methodNames);
  ...
  if (!startMatch || startMatch.index !== (firstKeywordMatch == null ? void 0 : firstKeywordMatch.index)) {
    return replaceObjectSnap(code, s, index5, newSnap, assertionName);
  }
  const quote = startMatch[1];
  const startIndex = index5 + startMatch.index + startMatch[0].length;
  const snapString = prepareSnapString(newSnap, code, index5);
  if (quote === ")") {
...
function stripSnapshotIndentation(inlineSnapshot) {
  ...
  for (let i4 = 1; i4 < lines.length - 1; i4++) {
    if (lines[i4] !== "") {
      if (lines[i4].indexOf(indentation) !== 0) {
        return inlineSnapshot;
      }
      lines[i4] = lines[i4].substring(indentation.length);
    }
  }
...
function isSameStackPosition(x, y4) {
  return x.file === y4.file && x.column === y4.column && x.line === y4.line;
}
var SnapshotState = class _SnapshotState {
  ...
  _inferInlineSnapshotStack(stacks) {
    const promiseIndex = stacks.findIndex((i4) => i4.method.match(/__VITEST_(RESOLVES|REJECTS)__/));
    if (promiseIndex !== -1) {
      return stacks[promiseIndex + 3];
    }
    const pollChainIndex = stacks.findIndex((i4) => i4.method.match(/__VITEST_POLL_CHAIN__/));
    if (pollChainIndex !== -1) {
      return stacks[pollChainIndex + 1];
    }
    for (let i4 = 0; i4 < stacks.length; i4++) {
      const match = stacks[i4].method.match(/__INLINE_SNAPSHOT_OFFSET_(\d+)__/);
      if (match) {
        return stacks[i4 + Number(match[1])] ?? null;
      }
    }
    const customMatcherIndex = stacks.findIndex((i4) => i4.method.includes("__VITEST_EXTEND_ASSERTION__"));
    if (customMatcherIndex !== -1) {
      return stacks[customMatcherIndex + 3] ?? null;
    }
    const stackIndex = stacks.findIndex((i4) => i4.method.includes("__INLINE_SNAPSHOT__"));
  ...
  _resolveKey(testId, testName4, key) {
...
${message}`);
  ...
  probeExpectedSnapshot(options) {
    ...
    return {
      key,
      count,
      data: (options == null ? void 0 : options.isInline) ? options.inlineSnapshot : this._snapshotData[key],
      markAsChecked: () => {
  ...
  match({ testId, testName: testName4, received, key, inlineSnapshot, isInline, error, rawSnapshot, assertionName }) {
    const resolved = this._resolveKey(testId, testName4, key);
    key = resolved.key;
  ...
  processDomainSnapshot({ testId, received, expectedSnapshot, matchResult, isInline, error, assertionName }) {
    ...
    const actualResolved = (matchResult == null ? void 0 : matchResult.resolved) ?? received;
    const expectedResolved = (matchResult == null ? void 0 : matchResult.expected) ?? expectedSnapshot.data;
    return this._reconcile({
      testId,
      key: expectedSnapshot.key,
      count: expectedSnapshot.count,
      pass: (matchResult == null ? void 0 : matchResult.pass) ?? false,
...
var SnapshotClient = class {
  ...
  match(options) {
    ...
    if (typeof properties === "object") {
      ...
      let propertiesPass;
      try {
        propertiesPass = ((_b = (_a = this.options).isEqual) == null ? void 0 : _b.call(_a, received, properties)) ?? false;
      } catch (err) {
        expectedSnapshot.markAsChecked();
        throw err;
      }
    ...
    return {
      ...
      actual: rawSnapshot ? actual : actual == null ? void 0 : actual.trim(),
      expected: rawSnapshot ? expected : expected == null ? void 0 : expected.trim()
    };
  }
  assert(options) {
  ...
  matchDomain(options) {
    ...
    return {
      ...
      actual: actual == null ? void 0 : actual.trim(),
      expected: expected == null ? void 0 : expected.trim()
    };
  }
  async pollMatchDomain(options) {
    ...
    expectedSnapshot.markAsChecked();
    if (!(stableResult == null ? void 0 : stableResult.rendered)) {
      if (stableResult == null ? void 0 : stableResult.lastPollError) {
...
// node_modules/vitest/dist/chunks/test.D1JkM1w4.js
var fakeTimersSrc = {};
var global2;
...
function requireCalledInOrder() {
  ...
  function checkAdjacentCalls(callMap, spy, index5, spies) {
    var calledBeforeNext = true;
    if (index5 !== spies.length - 1) {
      calledBeforeNext = spy.calledBefore(spies[index5 + 1]);
    }
...
function requireOrderByFirstCall() {
  ...
  function comparator(a, b4) {
    var aCall = a.getCall(0);
    var bCall = b4.getCall(0);
...
function requireFakeTimersSrc() {
  ...
  function withGlobal(_global) {
    ...
    function checkIsNearInfiniteLimit(clock, i4) {
      if (clock.loopLimit && i4 === clock.loopLimit - 1) {
    ...
    function parseTime(str) {
      ...
      while (i4--) {
        parsed = parseInt(strings[i4], 10);
        if (parsed >= 60) {
          throw new Error(`Invalid time ${str}`);
        }
        ms += parsed * Math.pow(60, l - i4 - 1);
      }
      return ms * 1e3;
    }
...
${job.error.stack.split("\n").slice(matchedLineIndex + 1).join("\n")}`;
    ...
    function createDate() {
      ...
      ClockDate.toString = function toString6() {
    ...
    function runJobs(clock) {
      ...
      for (let i4 = 0; i4 < clock.jobs.length; i4++) {
        const job = clock.jobs[i4];
        job.func.apply(null, job.args);
        checkIsNearInfiniteLimit(clock, i4);
        if (clock.loopLimit && i4 > clock.loopLimit) {
    ...
    function compareTimers(a, b4) {
      if (a.callAt < b4.callAt) {
        return -1;
      }
      if (a.callAt > b4.callAt) {
        return 1;
      }
      if (a.immediate && !b4.immediate) {
        return -1;
      }
      if (!a.immediate && b4.immediate) {
        return 1;
      }
      if (a.createdAt < b4.createdAt) {
        return -1;
      }
      if (a.createdAt > b4.createdAt) {
        return 1;
...
To automatically clean-up native timers, use \`shouldClearNativeTimers\`.`
    ...
    function uninstall(clock) {
      ...
      for (i4 = 0, l = clock.methods.length; i4 < l; i4++) {
        method = clock.methods[i4];
    ...
    function createClock(start, loopLimit) {
      ...
      if (typeof _global.Promise !== "undefined" && utilPromisify) {
        clock.setTimeout[utilPromisify.custom] = function promisifiedSetTimeout(timeout, arg) {
          return new _global.Promise(function setTimeoutExecutor(resolve6) {
            addTimer(clock, {
              func: resolve6,
              args: [arg],
              delay: timeout
        ...
        if (typeof _global.Promise !== "undefined" && utilPromisify) {
          clock.setImmediate[utilPromisify.custom] = function promisifiedSetImmediate(arg) {
            return new _global.Promise(
              function setImmediateExecutor(resolve6) {
                addTimer(clock, {
                  func: resolve6,
                  args: [arg],
                  immediate: true
      ...
      function doTick(tickValue, isAsync, resolve6, reject) {
      ...
      if (typeof _global.Promise !== "undefined") {
        clock.tickAsync = function tickAsync(tickValue) {
          return pauseAutoTickUntilFinished(
            new _global.Promise(function(resolve6, reject) {
              originalSetTimeout(function() {
                try {
                  doTick(tickValue, true, resolve6, reject);
                } catch (e) {
                  reject(e);
                }
              });
      ...
      clock.runAll = function runAll() {
        let numTimers, i4;
        runJobs(clock);
        for (i4 = 0; i4 < clock.loopLimit; i4++) {
          if (!clock.timers) {
            resetIsNearInfiniteLimit();
            return clock.now;
          }
    ...
    function install(config2) {
      ...
      for (i4 = 0, l = clock.methods.length; i4 < l; i4++) {
        const nameOfMethodToReplace = clock.methods[i4];
        ...
        if (clock.timersPromisesModuleMethods !== void 0) {
          if (nameOfMethodToReplace === "setTimeout") {
            ...
            timersPromisesModule.setTimeout = (delay2, value, options = {}) => new Promise((resolve6, reject) => {
              const abort = () => {
            ...
            timersPromisesModule.setImmediate = (value, options = {}) => new Promise((resolve6, reject) => {
              const abort = () => {
                options.signal.removeEventListener(
                  "abort",
                  abort
...
var FakeTimers = class {
  ...
  advanceTimersToNextTimer(steps = 1) {
    if (this._checkFakeTimers()) for (let i4 = steps; i4 > 0; i4--) {
      this._clock.next();
      this._clock.tick(0);
...
function waitFor(callback, options = {}) {
  const { setTimeout: setTimeout4, setInterval: setInterval2, clearTimeout: clearTimeout4, clearInterval: clearInterval2 } = getSafeTimers2();
...
function getImporter(name) {
  ...
  return ((_a = parseSingleStack2(stackArray[stackArray.findLastIndex((stack) => {
...
function createExpectPoll(expect2) {
  return function poll(fn2, options = {}) {
    ...
    const test7 = chai_exports.util.flag(assertion, "vitest-test");
    ...
    const proxy = new Proxy(assertion, { get(target, key, receiver) {
      ...
      return function __VITEST_POLL_CHAIN__(...args) {
        ...
        const promise = async () => {
          ...
          if ((_a2 = Object.getOwnPropertyDescriptor(assertionFunction, "__vitest_poll_takeover__")) == null ? void 0 : _a2.value) try {
          ...
          const { setTimeout: setTimeout4, clearTimeout: clearTimeout4 } = getSafeTimers2();
...
function getTest(obj) {
  const test7 = chai_exports.util.flag(obj, "vitest-test");
  if (!test7) throw new Error(`'${getAssertionName(obj)}' cannot be used without test context`);
  return test7;
}
...
function createExpect(test7) {
  ...
  setState({
    ...
    currentTestName: test7 ? test7.fullTestName ?? "" : globalState.currentTestName
  }, expect2);
...
async function runBenchmarkSuite(suite2, runner2) {
  ...
  if (benchmarkGroup.length) {
    ...
    for (const benchmark of benchmarkGroup) {
      ...
      await task.warmup();
      tasks.push([await new Promise((resolve6) => setTimeout4(async () => {
        resolve6(await task.run());
      })), benchmark]);
    }
...
var TestRunner = class {
  ...
  async onAfterRunSuite(suite2) {
    ...
    if (suite2.mode !== "skip" && "filepath" in suite2) {
      for (const test7 of getTests(suite2)) if (test7.mode === "skip") {
        const name = getNames(test7).slice(1).join(" > ");
...
`;
        ...
        suite2.result.errors.push(processError2(new Error(message)));
  ...
  onAfterRunTask(test7) {
    if (this.config.logHeapUsage && typeof process !== "undefined") test7.result.heap = process.memoryUsage().heapUsed;
    this.workerState.current = test7.suite || test7.file;
  }
  cancel(_reason) {
    this.cancelRun = true;
  }
  ...
  async onBeforeRunTask(test7) {
    if (this.cancelRun) test7.mode = "skip";
    if (test7.mode !== "run" && test7.mode !== "queued") return;
    this.workerState.current = test7;
  }
  ...
  onBeforeTryTask(test7) {
    clearModuleMocks(this.config);
    this.snapshotClient.clearTest(test7.file.filepath, test7.id);
    setState({
      assertionCalls: 0,
      isExpectingAssertions: false,
      isExpectingAssertionsError: null,
      expectedAssertionsNumber: null,
      expectedAssertionsNumberErrorGen: null,
      currentTestName: getTestName(test7),
      snapshotState: this.snapshotClient.getSnapshotState(test7.file.filepath)
    }, globalThis[GLOBAL_EXPECT]);
  }
  onAfterTryTask(test7) {
    const { assertionCalls, expectedAssertionsNumber, expectedAssertionsNumberErrorGen, isExpectingAssertions, isExpectingAssertionsError } = test7.context._local ? test7.context.expect.getState() : getState(globalThis[GLOBAL_EXPECT]);
    ...
    if (this.config.expect.requireAssertions && assertionCalls === 0) throw this.assertionsErrors.get(test7);
  }
  ...
  getImportDurations() {
    ...
    if (limit === 0) return {};
    const sortedEntries = [...((_a = this.workerState.moduleExecutionInfo) == null ? void 0 : _a.entries()) || []].sort(([, a], [, b4]) => b4.duration - a.duration).slice(0, limit);
...
var _DRIVE_LETTER_START_RE5 = /^[A-Za-z]:\//;
function normalizeWindowsPath5(input = "") {
  return input && input.replace(/\\/g, "/").replace(_DRIVE_LETTER_START_RE5, (r) => r.toUpperCase());
}
var _IS_ABSOLUTE_RE5 = /^[/\\](?![/\\])|^[/\\]{2}(?!\.)|^[A-Za-z]:[/\\]/;
var _DRIVE_LETTER_RE2 = /^[A-Za-z]:$/;
function cwd5() {
  return typeof process < "u" && typeof process.cwd == "function" ? process.cwd().replace(/\\/g, "/") : "/";
}
var resolve5 = function(...arguments_) {
  arguments_ = arguments_.map((argument) => normalizeWindowsPath5(argument));
  let resolvedPath = "", resolvedAbsolute = false;
  for (let index5 = arguments_.length - 1; index5 >= -1 && !resolvedAbsolute; index5--) {
    let path = index5 >= 0 ? arguments_[index5] : cwd5();
    !path || path.length === 0 || (resolvedPath = `${path}/${resolvedPath}`, resolvedAbsolute = isAbsolute5(path));
  }
  return resolvedPath = normalizeString5(resolvedPath, !resolvedAbsolute), resolvedAbsolute && !isAbsolute5(resolvedPath) ? `/${resolvedPath}` : resolvedPath.length > 0 ? resolvedPath : ".";
};
function normalizeString5(path, allowAboveRoot) {
  let res = "", lastSegmentLength = 0, lastSlash = -1, dots = 0, char = null;
  for (let index5 = 0; index5 <= path.length; ++index5) {
    if (index5 < path.length) char = path[index5];
    else if (char === "/") break;
    else char = "/";
    if (char === "/") {
      if (!(lastSlash === index5 - 1 || dots === 1)) if (dots === 2) {
        if (res.length < 2 || lastSegmentLength !== 2 || res[res.length - 1] !== "." || res[res.length - 2] !== ".") {
          if (res.length > 2) {
            let lastSlashIndex = res.lastIndexOf("/");
            lastSlashIndex === -1 ? (res = "", lastSegmentLength = 0) : (res = res.slice(0, lastSlashIndex), lastSegmentLength = res.length - 1 - res.lastIndexOf("/")), lastSlash = index5, dots = 0;
            continue;
          } else if (res.length > 0) {
            res = "", lastSegmentLength = 0, lastSlash = index5, dots = 0;
            continue;
          }
        }
        allowAboveRoot && (res += res.length > 0 ? "/.." : "..", lastSegmentLength = 2);
      } else res.length > 0 ? res += `/${path.slice(lastSlash + 1, index5)}` : res = path.slice(lastSlash + 1, index5), lastSegmentLength = index5 - lastSlash - 1;
      lastSlash = index5, dots = 0;
    } else char === "." && dots !== -1 ? ++dots : dots = -1;
  }
  return res;
}
var isAbsolute5 = function(p) {
  return _IS_ABSOLUTE_RE5.test(p);
};
var dirname2 = function(p) {
  let segments = normalizeWindowsPath5(p).replace(/\/$/, "").split("/").slice(0, -1);
  return segments.length === 1 && _DRIVE_LETTER_RE2.test(segments[0]) && (segments[0] += "/"), segments.join("/") || (isAbsolute5(p) ? "/" : ".");
};
...
for (let i4 = 0; i4 < chars4.length; i4++) {
  let c = chars4.charCodeAt(i4);
  intToChar4[i4] = c, charToInt4[c] = i4;
}
...
// node_modules/vitest/dist/chunks/index.C-zNJvRx.js
